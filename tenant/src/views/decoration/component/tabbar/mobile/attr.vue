...
const normalizeTabbar = () => {
    ...
    data.value.list.forEach((item: any) => {
        item.is_show = item.is_show == 1 ? 1 : 0
    })
    const visibleList = data.value.list.filter((tab: any) => tab.is_show == 1)
    if (data.value.list.length >= min && visibleList.length < min) {
        data.value.list.slice(0, min).forEach((item: any) => {
            item.is_show = 1
        })
    }
