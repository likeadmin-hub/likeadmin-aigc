...
<script lang="ts" setup name="tenant-aigc-video-case">
import {
    deleteAigcVideoCase,
    getAigcVideoCases,
    saveAigcVideoCase,
...
const handleStatus = async (row: any) => {
    await setAigcVideoCaseStatus({ id: row.id, status: row.status })
}
const handleDelete = async (id: number) => {
    await feedback.confirm('确定删除该案例？')
    await deleteAigcVideoCase({ id })
    getLists()
}
