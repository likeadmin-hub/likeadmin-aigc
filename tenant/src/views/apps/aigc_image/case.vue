        ...
        <el-card class="!border-none mt-4" shadow="never">
            <el-table v-loading="loading" size="large" :data="lists">
                ...
                <el-table-column label="应用" width="110">
                    <template #default="{ row }">
                        <el-tag :type="row.app_code === 'aigc_video' ? 'warning' : 'success'">
                            {{ appName(row.app_code) }}
                        </el-tag>
                    </template>
                </el-table-column>
            ...
            <el-form label-width="100px" :model="formData">
                <el-form-item label="应用">
                    <el-radio-group v-model="formData.app_code">
                        <el-radio value="aigc_image">AIGC生图</el-radio>
                        <el-radio value="aigc_video">AIGC视频</el-radio>
                    </el-radio-group>
                </el-form-item>
...
<script lang="ts" setup name="tenant-aigc-image-case">
import {
    deleteAigcImageCase,
    getAigcImageCases,
    saveAigcImageCase,
    setAigcImageCaseStatus
...
const getLists = async () => {
    loading.value = true
    try {
        lists.value = await getAigcImageCases(queryParams)
    } finally {
        loading.value = false
    }
}
...
const handleStatus = async (row: any) => {
    await setAigcImageCaseStatus({ id: row.id, status: row.status })
}
const handleDelete = async (id: number) => {
    await feedback.confirm('确定删除该案例？')
    await deleteAigcImageCase({ id })
    getLists()
}
const appName = (appCode: string) => {
