                ...
                <el-table-column label="结果图" width="180">
                    <template #default="{ row }">
                        <div v-if="row.results?.length" class="flex flex-wrap gap-2">
                            <el-image
                                v-for="item in row.results.slice(0, 3)"
                                :key="item.id"
                                class="w-[52px] h-[52px] rounded"
                                :src="item.image_url"
                                :preview-src-list="row.results.map((result: any) => result.image_url).filter(Boolean)"
                                preview-teleported
                                fit="cover"
                            />
                        </div>
...
const getLists = async () => {
    loading.value = true
    try {
        lists.value = await getAigcImageTaskLists(queryParams)
    } finally {
        loading.value = false
    }
}
