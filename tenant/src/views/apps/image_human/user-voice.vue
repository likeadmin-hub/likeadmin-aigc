...
<script lang="ts" setup name="tenant-image-human-user-voice">
import {
    deleteImageHumanUserVoice,
    getImageHumanUserVoices,
    publishImageHumanUserVoice
...
const { pager, getLists, resetPage } = usePaging({
    fetchFun: getImageHumanUserVoices,
    params: query
})
const resetQuery = () => {
...
const handlePublish = async (id: number) => {
    ...
    await publishImageHumanUserVoice({ id })
    feedback.msgSuccess('已设为公共音色')
    getLists()
}
const handleDelete = async (id: number) => {
    await feedback.confirm('确定删除该用户音色？')
    await deleteImageHumanUserVoice({ id })
    getLists()
}
