...
<script lang="ts" setup name="tenant-image-human-stat">
import { getImageHumanStat } from '@/apps/image_human/api'
...
const cards = computed(() => [
    ...
    { label: '共享音色', value: stat.value.voice_total || 0 },
...
const getData = async () => {
    loading.value = true
    try {
        stat.value = await getImageHumanStat()
    } finally {
        loading.value = false
    }
}
