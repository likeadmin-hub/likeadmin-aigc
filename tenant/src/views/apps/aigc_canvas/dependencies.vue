<template>
    <el-card class="!border-none" shadow="never" v-loading="loading">
        <el-alert class="mb-4" type="info" show-icon :closable="false" title="无限画布直接复用 AIGC生图 和 AIGC视频 的通道、模型、计费和任务配置，本应用不单独配置 API。" />
        <el-table :data="dependencies.items || []" size="large">
            ...
            <el-table-column label="租户状态" width="140">
                <template #default="{ row }">
                    <el-tag :type="row.tenant_enabled ? 'success' : 'danger'">{{ row.tenant_enabled ? '已开通上架' : '不可用' }}</el-tag>
                </template>
            </el-table-column>
