    ...
    <el-card class="!border-none" shadow="never">
        <el-form :inline="true" class="mb-4">
            ...
            <el-form-item label="关键词">
                <el-input v-model="query.keyword" class="w-[220px]" clearable placeholder="形象名称" />
            </el-form-item>
        ...
        <el-table v-loading="loading" size="large" :data="lists">
            ...
            <el-table-column label="封面" width="110">
                <template #default="{ row }">
                    <video v-if="isVideo(row.cover_url || row.media_url)" class="w-[64px] h-[64px] rounded object-cover bg-[#f5f7fa]" :src="row.cover_url || row.media_url" muted preload="metadata" />
                    <el-image v-else class="w-[64px] h-[64px] rounded" :src="row.cover_url || row.media_url" fit="cover" />
                </template>
            </el-table-column>
...
import { deleteAigcDigitalHumanUserAvatar, getAigcDigitalHumanUserAvatars } from '@/apps/aigc_digital_human/api'
...
const getLists = async () => {
    loading.value = true
    try {
        lists.value = await getAigcDigitalHumanUserAvatars(query)
    } finally {
        loading.value = false
    }
}
