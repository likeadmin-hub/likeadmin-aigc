<template>
    <div>
        ...
        <el-card class="!border-none mt-4" shadow="never">
            ...
            <el-table-column label="合成视频" width="150">
            ...
            <el-table-column label="封面下载地址" min-width="240" show-overflow-tooltip>
                <template #default="{ row }">
                    <el-link v-if="row.cover_url" type="primary" :href="row.cover_url" target="_blank">
                        {{ row.cover_url }}
                    </el-link>
                    <span v-else>-</span>
                </template>
            </el-table-column>
            <el-table-column label="TTS任务ID" prop="tts_task_id" min-width="180" show-overflow-tooltip />
            <el-table-column label="视频任务ID" prop="provider_task_id" min-width="180" show-overflow-tooltip />
...
const getLists = async () => {
    loading.value = true
    try {
        lists.value = await getAigcDigitalHumanTaskLists(queryParams)
    } finally {
        loading.value = false
    }
}
