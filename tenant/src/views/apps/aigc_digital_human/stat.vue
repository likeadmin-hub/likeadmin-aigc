...
<script lang="ts" setup name="tenant-aigc-digital-human-stat">
import { getAigcDigitalHumanStat } from '@/apps/aigc_digital_human/api'
...
const getData = async () => {
    loading.value = true
    try {
        stat.value = await getAigcDigitalHumanStat()
    } finally {
        loading.value = false
    }
}
