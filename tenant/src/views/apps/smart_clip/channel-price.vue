<template>
    <div class="aigc-price-page">
        <el-card class="!border-none price-card" shadow="never">
            ...
            <el-empty v-if="!visibleChannels.length && !loading" description="暂无剪辑通道" />
            <template v-else>
...
<script lang="ts" setup name="tenant-smart-clip-channel-price">
import { batchSaveSmartClipChannels, getSmartClipChannels } from '@/apps/smart_clip/api'
...
const getLists = async () => {
    loading.value = true
    try {
        channels.value = await getSmartClipChannels()
    } finally {
        loading.value = false
    }
}
...
const saveSpecs = async (rows: any[]) => {
    ...
    saving.value = true
    try {
        await batchSaveSmartClipChannels({
            specs: rows.map((row) => ({
