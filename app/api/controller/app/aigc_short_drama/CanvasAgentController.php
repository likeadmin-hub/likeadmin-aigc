...
final class CanvasAgentController extends BaseApiController
{
    ...
    public function confirmWorkflowPlan() { return $this->respond(function () {
        $p=$this->request->post();
        if (array_diff(array_keys($p),['canvas_id','thread_id','expected_revision','plan_hash']) || !is_string($p['plan_hash']??null)) throw new RuntimeException('UNSUPPORTED_MESSAGE_FIELD');
        return ConversationWorkflow::confirmPlan((int)$this->request->tenantId,$this->userId,self::number($p['canvas_id']??null),self::number($p['thread_id']??null),self::number($p['expected_revision']??null,true),$p['plan_hash']);
    }); }

    /** Confirm a non-billable structured text plan before it writes canvas nodes. */
    public function confirmWorkflowStage() { return $this->respond(function () {
        $p=$this->request->post();
        if (array_diff(array_keys($p),['canvas_id','thread_id','expected_revision'])) throw new RuntimeException('UNSUPPORTED_MESSAGE_FIELD');
        return ConversationWorkflow::confirmStagePlan((int)$this->request->tenantId,$this->userId,self::number($p['canvas_id']??null),self::number($p['thread_id']??null),self::number($p['expected_revision']??null,true));
    }); }
    ...
    private function respond(\Closure $action) {
        try {return $this->success('success',$action());}
            ...
            $public=['CANVAS_NOT_FOUND','THREAD_NOT_FOUND','RUN_NOT_FOUND','CANVAS_AGENT_DISABLED','CANVAS_AGENT_EXECUTION_DISABLED','IDEMPOTENCY_CONFLICT','THREAD_BUSY','VERSION_CONFLICT','PREFERENCE_VERSION_CONFLICT','INVALID_PREFERENCE_REVISION','NODE_NOT_FOUND','INVALID_IDENTIFIER','INVALID_THREAD_REQUEST','INVALID_MESSAGE','INVALID_NODE_REFERENCES','INVALID_BASE_REVISION','UNSUPPORTED_MESSAGE_FIELD','CONTEXT_TOO_LARGE','INVALID_ATTACHMENTS','IMAGE_REFERENCE_UNAVAILABLE','TOO_MANY_IMAGE_REFERENCES','INVALID_AGENT_PREFERENCES','INVALID_GENERATION_MODE','INVALID_MODEL_SELECTION','REASONING_MODEL_UNAVAILABLE','IMAGE_MODEL_UNAVAILABLE','VIDEO_MODEL_UNAVAILABLE','INVALID_SKILL_SELECTION','SKILL_UNAVAILABLE','CONTENT_BLOCKED','DOCUMENT_NOT_FOUND','DOCUMENT_NOT_READY','DOCUMENT_INVALID_STATE','DOCUMENT_QUOTE_EXPIRED','DOCUMENT_URL_UNAVAILABLE','DOCUMENT_PARSE_SUBMIT_FAILED','DOCUMENT_PARSE_RECORD_MISSING','WORKFLOW_NOT_ACTIVE','WORKFLOW_VERSION_CONFLICT','WORKFLOW_SLOT_OUT_OF_ORDER','WORKFLOW_STAGE_NOT_COLLECTING','INVALID_WORKFLOW_ANSWER','INVALID_WORKFLOW_PLAN','WORKFLOW_PLAN_STALE','WORKFLOW_PLAN_CONFIRMATION_REQUIRED','WORKFLOW_IMAGE_PLAN_REQUIRED','WORKFLOW_IMAGE_MODEL_UNAVAILABLE','WORKFLOW_IMAGE_QUOTE_UNAVAILABLE','WORKFLOW_PLAN_SOURCE_CHANGED','WORKFLOW_STAGE_PLAN_STALE','WORKFLOW_STAGE_PLAN_REQUIRED','WORKFLOW_SKILL_UNAVAILABLE','INVALID_WORKFLOW_STATE','INVALID_WORKFLOW_AUTO_REQUEST','INVALID_WORKFLOW_AUTO_CONTEXT','INVALID_WORKFLOW_AUTO_FLAG'];
    ...
    {
        $code=$error->getMessage();
        $public=['CANVAS_NOT_FOUND','THREAD_NOT_FOUND','RUN_NOT_FOUND','CANVAS_AGENT_DISABLED','CANVAS_AGENT_EXECUTION_DISABLED','INVALID_IDENTIFIER','UNSUPPORTED_MESSAGE_FIELD'];
