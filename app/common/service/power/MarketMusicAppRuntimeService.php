...
class MarketMusicAppRuntimeService
{
    ...
    public static function options(int $tenantId): array
    {
        ...
        foreach ($products as $product) {
            ...
            foreach ($skus as $sku) {
                ...
                if ((string)($locked['type'] ?? '') !== self::GENERATE_TYPE) {
                    continue;
                }
                ...
                $tenantPrice = $tenant->isEmpty()
                    ? (float)$sku['sale_points']
                    : (float)$tenant['sale_points'];
                $duration = (int)($locked['duration'] ?? $locked['seconds'] ?? 0);
                ...
                $options[] = [
                    ...
                    // A duration selector is only valid when the priced SKU
    ...
    ): array
    {
        ...
        return Db::transaction(function () use ($tenantId, $userId, $businessTaskId, $request, $market, $tenantCost, $userPrice, $deferredUsage, $appCode, $actionCode, $businessTable) {
            ...
            $appTask = AiAppTask::create([
                ...
                'app_code' => $appCode, 'action_code' => $actionCode,
            ...
            $consumption = AiConsumptionLog::create([
                ...
                'app_code' => $appCode, 'action_code' => $actionCode, 'resource_type' => 'app_api',
    ...
    public static function fail(int $consumptionId, string $message, string $code = 'failed'): void
    {
        Db::transaction(function () use ($consumptionId, $message, $code) {
            ...
            if ((float)$c['reserved_tenant_cost'] > 0 || (float)$c['reserved_user_price'] > 0) PointService::releaseReservedBusinessAmountsInCurrentTransaction((int)$c['tenant_id'], (int)$c['user_id'], (float)$c['reserved_tenant_cost'], (float)$c['reserved_user_price'], (string)$c['consume_no'] . '-release', self::taskLabel((string)($task['app_code'] ?? self::APP_CODE)) . '失败退回', self::extra($task, $c, 'refunded'));
            $now = time();
    ...
    private static function payload(array $snapshot, array $request, string $idempotencyKey): array
    {
        ...
        $duration = (int)($locked['duration'] ?? $locked['seconds'] ?? $providerParams['duration'] ?? $request['duration'] ?? 60);
        foreach (['title', 'prompt', 'lyrics', 'instrumental', 'custom', 'genre', 'mood', 'instruments', 'duration'] as $key) {
            unset($params[$key]);
        }
        $payload = array_merge($params, self::marketContext($snapshot), [
            ...
            'instruments' => trim((string)($request['instruments'] ?? '')), 'duration' => max(5, min(600, $duration)),
