    ...
    public static function process(int $tenant,int $user,int $run,ConversationProviderInterface $provider): string
    {
        ...
        try {
            ...
            if ($responseFormat!==null) {
                ...
                $request['max_tokens']=$activeRouting ? ($workflowStage==='script' ? 8192 : ($workflowStage==='video_nodes' ? 8192 : 4096)) : ($intentRouting ? ((int)($intentRouting['version']??2)>=3 ? 4096 : 1800) : ($intakeAnalysis ? 1500 : ($compact && $workflowStage==='script' ? 8192 : 4096)));
