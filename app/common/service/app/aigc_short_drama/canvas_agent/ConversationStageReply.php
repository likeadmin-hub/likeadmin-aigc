...
final class ConversationStageReply
{
    public static function present(array $workflow, string $reply, array $nodes, bool $written): string
    {
        ...
        $looksLikeArtifact=mb_strlen($reply)>260
            ...
            || ($written && in_array($stage,['assets','storyboard'],true) && preg_match('/确认后再|预计积分|预估积分|图片计划/u',$reply));
        if (mb_strlen($reply)>=20 && !$looksLikeArtifact) {
            if (!preg_match('/请确认|接下来|下一步|继续|逐个/u',$reply)) $reply.=' '.self::nextStep($stage,$written);
            return $reply;
        }
        ...
        return match ($stage) {
            ...
            'assets'=>$written?'已在画布插入'.count($nodes).'个主体相关图片节点'.$detail.'。请在需要生成的节点逐个确认图片任务。':'已准备'.count($nodes).'个主体相关图片节点'.$detail.'。请核对图片计划与预估积分；确认后再插入画布并按模式生成。',
            'storyboard'=>$written?'已在画布插入'.count($nodes).'个场景或分镜图片节点'.$detail.'。请在需要生成的节点逐个确认图片任务。':'已准备'.count($nodes).'个场景或分镜图片节点'.$detail.'。请核对所用素材与预估积分；确认后再生成图片。',
    ...
    private static function nextStep(string $stage,bool $written): string
    {
        return match ($stage) {
            'script'=>'请确认故事和单集剧本，随后继续美术规划。',
            'art'=>'请确认美术方向，随后准备主体图片。',
            'assets','storyboard'=>$written?'图片节点已插入画布，请按需逐个确认生成。':'请核对图片计划与预计积分，再决定是否生成。',
