...
final class ConversationActionPlan
{
    ...
    public static function instruction(string $mode,string $workflowStage='',bool $compact=false,bool $stageGenerationPrompts=false): string
    {
        if ($compact) {
            ...
            if ($workflowStage==='video_plan') return self::structuredEnvelopeInstruction('分镜视频规划', 'video_prompt_plan', '每镜包含 shot_number、duration、first_frame、last_frame、camera_motion、action_sequence、video_prompt、asset_references；完成后自动保存在对话工作流状态，不创建画布文本节点。');
            ...
            if ($stageGenerationPrompts && $workflowStage==='video_nodes') return self::mediaEnvelopeInstruction('当前为分镜视频节点阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 video，artifact 为 storyboard_video；每项 reference_keys 必须从 workflow_reference_catalog 选择唯一对应分镜图，可额外选实际需要的主体/场景图片，不得连接无关节点或以 depends_on 串联视频。每项 prompt 必须是该镜头独立返回的中文 video_prompt，直接显示在视频节点输入框；采用原短剧的六行导演稿：分镜编号与时间、景别、构图、运镜手法、画面内容、声音。声音一行必须保留本镜头实际说话主体、对白原文、必要旁白与动作音效；无对白的镜头明确写无对白，不得把这些内容转移到音频规划节点。背景音乐单独放入音频规划节点，不在逐镜视频提示词重复配乐生成要求。不要写后端执行标签、英文生成描述或整份视频规划。只插入待用户生成节点，不自动报价、提交或计费；最多六十项。每项只允许 type、artifact、title、prompt、key、reference_keys；不得声明价格、模型、URL、素材 ID 或任务状态。');
            ...
            if ($workflowStage==='video_nodes') return '当前为分镜视频节点阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 video，artifact 为 storyboard_video；每项 reference_keys 必须从 workflow_reference_catalog 选择对应分镜图，可额外选实际需要的主体/场景图片，不得连接无关节点或以 depends_on 串联视频。逐镜提示词的声音内容包含本镜头实际说话主体、对白、旁白与必要音效；无对白写明无对白。背景音乐另交音频规划节点。只插入待用户生成节点，不自动报价、提交或计费；最多六十项。每项只允许 type、artifact、title、prompt、key、reference_keys；不得声明价格、模型、URL、素材 ID 或任务状态。';
        }
        ...
        if ($workflowStage==='video_nodes') return self::mediaEnvelopeInstruction("当前为分镜视频节点阶段。只在答复末尾输出一次严格 JSON 包裹 <canvas-actions>{\"nodes\":[...]}</canvas-actions>；nodes 只能是 video，数量 1 至 60，artifact 必须是 storyboard_video，全部为待用户生成的分镜视频节点。逐镜 prompt 的声音内容必须包含本镜头实际说话主体、对白、必要旁白和动作音效；无对白写明无对白。背景音乐另交音频规划节点。每项只允许 type、artifact、title、prompt、key、depends_on、reference_keys；reference_keys 只能选用阶段输入 workflow_reference_catalog 的 reference_key。不得声明价格、模型、URL、素材 ID 或任务状态。");
