...
final class ConversationActionPlan
{
    ...
    public static function instruction(string $mode,string $workflowStage='',bool $compact=false,bool $stageGenerationPrompts=false): string
    {
        ...
        if ($workflowStage==='audio_plan') return self::mediaEnvelopeInstruction("当前为音频规划阶段。只在答复末尾输出一次严格 JSON 包裹 <canvas-actions>{\"nodes\":[{\"type\":\"audio\",\"artifact\":\"audio_plan\",\"title\":\"音频规划（暂未开放）\",\"prompt\":\"...\",\"key\":\"audio_plan\"}]}</canvas-actions>；只允许一个 audio 节点，每项只能有 type、artifact、title、prompt、key 五个字段。prompt 必须是单个字符串，根据已确认单集剧本逐镜写出对白、旁白、环境声、音效和配乐情绪的真实规划，不得返回对象、数组或占位符。此节点仅展示规划，绝不能生成或计费。");
