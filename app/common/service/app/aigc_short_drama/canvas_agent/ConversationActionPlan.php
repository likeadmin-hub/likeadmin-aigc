...
final class ConversationActionPlan
{
    ...
    public static function instruction(string $mode,string $workflowStage='',bool $compact=false,bool $stageGenerationPrompts=false): string
    {
        if ($compact) {
            ...
            if ($stageGenerationPrompts && $workflowStage==='storyboard') return self::mediaEnvelopeInstruction('当前为场景与分镜图阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 image，artifact 为 scene 或 storyboard；每个 scene 的 reference_keys 必须且只能包含该场景对应的一项 art 阶段 scene_image_prompt；服务端会把这项已确认提示词原样放进场景图输入框，不要重写。先逐条读取已确认单集剧本中按时间顺序编号的镜头：每个剧本镜头恰好对应一个同编号 storyboard 节点，标题写明镜头编号；严格保留该镜头的人物、地点、可见动作、因果顺序与结局，不得把其他镜头的动作提前、合并或凭空改写。每个 storyboard 必须 depends_on 同批对应 scene，reference_keys 只选该镜头确实出镜或决定画面一致性的主体图/三视图。storyboard 的 prompt 必须是这一镜头独立返回的中文 image_prompt，即实际提交的生图内容，只描述当前可见主体、动作、场景、构图、光线与画风，不要写策划说明或追加整份规划。关键道具写入该镜头 prompt，不默认创建道具图。必须覆盖已确认单集剧本的每个实际镜头，场景复用而非每镜重复创建；最多二十四项。每项只允许 type、artifact、title、prompt、key、depends_on、reference_keys；不得声明价格、模型、URL、素材 ID 或任务状态。');
            if ($stageGenerationPrompts && $workflowStage==='video_nodes') return self::mediaEnvelopeInstruction('当前为分镜视频节点阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 video，artifact 为 storyboard_video；每项 reference_keys 必须从 workflow_reference_catalog 选择唯一对应分镜图，可额外选实际需要的主体/场景图片，不得连接无关节点或以 depends_on 串联视频。每项 duration_seconds 必须是已确认分镜视频规划中同编号镜头的秒数，不得套用空白节点初始时长；prompt 第一行时间范围须与 duration_seconds 一致。每项 prompt 必须是该镜头独立返回的中文 video_prompt，直接显示在视频节点输入框；采用原短剧的六行导演稿：分镜编号与时间、景别、构图、运镜手法、画面内容、声音。声音一行必须保留本镜头实际说话主体、对白原文、必要旁白与动作音效；无对白的镜头明确写无对白，不得把这些内容转移到音频规划节点。背景音乐单独放入音频规划节点，不在逐镜视频提示词重复配乐生成要求。不要写后端执行标签、英文生成描述或整份视频规划。只插入待用户生成节点，不自动报价、提交或计费；最多六十项。每项只允许 type、artifact、title、prompt、key、reference_keys、duration_seconds；不得声明价格、模型、URL、素材 ID 或任务状态。');
            ...
            if ($workflowStage==='storyboard') return self::mediaEnvelopeInstruction('当前为场景与分镜图阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 image，artifact 为 scene 或 storyboard；每个 storyboard 必须 depends_on 同批对应 scene，且 reference_keys 只能从 workflow_reference_catalog 中选择该镜头确实出镜或决定画面一致性的主体图/三视图。逐条对齐已确认单集剧本的镜头编号与时间顺序，每个剧本镜头恰好对应一个同编号 storyboard；保留该镜头的人物、地点、可见动作、因果顺序与结局，不得添加替代情节。关键道具写入该镜头 prompt，不默认创建道具图。每个 prompt 须包含真实场景、角色、动作、构图和画风；必须覆盖已确认单集剧本的每个实际镜头，场景复用，最多二十四项。每项只允许 type、artifact、title、prompt、key、depends_on、reference_keys；不得声明价格、模型、URL、素材 ID 或任务状态。');
            if ($workflowStage==='video_nodes') return '当前为分镜视频节点阶段。只输出 <canvas-actions>{"nodes":[...]}</canvas-actions>。节点只能为 video，artifact 为 storyboard_video；每项 reference_keys 必须从 workflow_reference_catalog 选择对应分镜图，可额外选实际需要的主体/场景图片，不得连接无关节点或以 depends_on 串联视频。每项 duration_seconds 必须等于已确认视频规划的同编号镜头时长，prompt 时间范围与之对应，不得使用空白节点初始时长。逐镜提示词的声音内容包含本镜头实际说话主体、对白、旁白与必要音效；无对白写明无对白。背景音乐另交音频规划节点。只插入待用户生成节点，不自动报价、提交或计费；最多六十项。每项只允许 type、artifact、title、prompt、key、reference_keys、duration_seconds；不得声明价格、模型、URL、素材 ID 或任务状态。';
        }
        ...
        if ($workflowStage==='video_nodes') return self::mediaEnvelopeInstruction("当前为分镜视频节点阶段。只在答复末尾输出一次严格 JSON 包裹 <canvas-actions>{\"nodes\":[...]}</canvas-actions>；nodes 只能是 video，数量 1 至 60，artifact 必须是 storyboard_video，全部为待用户生成的分镜视频节点。每项 duration_seconds 必须等于已确认视频规划的同编号镜头时长，prompt 时间范围与之对应，不得使用空白节点初始时长。逐镜 prompt 的声音内容必须包含本镜头实际说话主体、对白、必要旁白和动作音效；无对白写明无对白。背景音乐另交音频规划节点。每项只允许 type、artifact、title、prompt、key、depends_on、reference_keys、duration_seconds；reference_keys 只能选用阶段输入 workflow_reference_catalog 的 reference_key。不得声明价格、模型、URL、素材 ID 或任务状态。");
        if ($workflowStage==='audio_plan') return self::mediaEnvelopeInstruction("当前为背景音乐规划阶段。只在答复末尾输出一次严格 JSON 包裹 <canvas-actions>{\"nodes\":[{\"type\":\"audio\",\"artifact\":\"audio_plan\",\"title\":\"背景音乐（暂未开放）\",\"prompt\":\"...\",\"key\":\"audio_plan\"}]}</canvas-actions>；只允许一个 audio 节点，每项只能有 type、artifact、title、prompt、key 五个字段。prompt 必须是单个可直接用于生成整部短剧纯背景音乐的中文字符串，依据已确认剧情描述音乐风格、情绪曲线、节奏/BPM、乐器、时长和循环/衔接要求。不得包含逐镜对白、角色说话、旁白、配音、环境音或音效规划；不要歌词和人声。上述逐镜声音内容应已写入对应分镜视频 prompt。不得返回对象、数组或占位符。此节点仅展示音乐提示词，绝不能生成或计费。");
        ...
        if ($workflowStage==='storyboard') return self::mediaEnvelopeInstruction("当前为场景与分镜图阶段。只在答复末尾输出严格 JSON 包裹 <canvas-actions>{\"nodes\":[...]}</canvas-actions>；nodes 只能是 image，artifact 只能是 scene、prop 或 storyboard；storyboard 必须通过 depends_on 引用前面同批的 scene 或 prop。reference_keys 只能选 workflow_reference_catalog 中直接决定本节点的 reference_key，不得把所有前序剧本和资产都连接到同一节点。逐条对齐已确认单集剧本的镜头编号与时间顺序，每个剧本镜头恰好对应一个同编号 storyboard；保留该镜头的人物、地点、可见动作、因果顺序与结局，不得添加替代情节。必须覆盖已确认单集剧本的每个实际镜头，最多二十四项，每项只允许 type、artifact、title、prompt、key、depends_on、reference_keys。不得声明价格、模型、URL、素材 ID 或任务状态。");
        if ($workflowStage==='video_plan') return self::structuredEnvelopeInstruction('分镜视频规划', 'video_prompt_plan', '每项 prompt 必须覆盖 shot_number、duration、first_frame、last_frame、camera_motion、action_sequence、video_prompt、asset_references；时长由本镜可见动作、对白、运镜和停顿决定，不按默认秒数或整集预算拉长单一画面；只规划，不报价、不提交视频任务。');
        $delivery = $mode === 'auto'
    ...
    public static function failureCategory(string $reply,string $stage,bool $compact=false): string
    {
        ...
        foreach ($action['nodes'] as $node) {
            if ($stage==='audio_plan' && is_array($node)) $node=array_intersect_key($node,array_flip(['type','artifact','title','prompt','key']));
            if (!is_array($node) || array_diff(array_keys($node),['type','artifact','title','prompt','key','depends_on','reference_keys','formal_fields','duration_seconds'])) return 'action_node_fields';
    ...
    public static function parse(string $reply,string $workflowStage='',bool $compact=false): array
    {
        ...
        foreach ($action['nodes'] as $node) {
            ...
            if ($workflowStage==='video_nodes' && array_key_exists('duration_seconds',$node)) {
                $duration=$node['duration_seconds'];
                if (!is_numeric($duration) || !is_finite((float)$duration) || (float)$duration<=0 || (float)$duration>60) throw new RuntimeException('INVALID_AGENT_ACTION');
                $proposal['duration_seconds']=(float)$duration;
            }
            if ($allowedArtifacts) {
