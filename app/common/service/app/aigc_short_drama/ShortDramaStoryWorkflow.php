...
final class ShortDramaStoryWorkflow
{
    ...
    public static function storyContext(array $data, array $request): array
    {
        ...
        if (ShortDramaEpisodeDuration::active($request)) $data['story_scale']['episode_duration_policy'] = ShortDramaEpisodeDuration::policy($request);
        return $data;
    }
    ...
    public static function scopeInstruction(array $request): string
    {
        ...
        if (($request['multi_episode_stage'] ?? '') === 'story') {
            ...
            if (ShortDramaEpisodeDuration::active($request)) {
                $durationHint = ShortDramaEpisodeDuration::instruction($request)
                    . '当前仅据冻结的时间策略评估故事容量，不提前生成分镜或在故事设定阶段分配逐集时长。';
            }
