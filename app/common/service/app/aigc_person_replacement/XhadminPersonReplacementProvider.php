...
class XhadminPersonReplacementProvider
{
    private const SUBMIT_PATH = '/api/v1/apps/person_replacement/submit';
    private const QUERY_PATH = '/api/v1/apps/person_replacement/query';

    public function submit(array $payload, array $context = []): PersonReplacementGenerateResult
    {
        try {
            $config = $this->resolveConfig();
            $payload = array_filter(array_merge(['type' => 'person_replacement'], $payload), static fn($value) => $value !== null && $value !== '' && $value !== []);
            $data = $this->request('POST', $config['base_url'] . self::SUBMIT_PATH, $config['api_key'], $payload, $config);
            $taskId = $this->extractTaskId($data);
            if ($taskId === '') {
                return new PersonReplacementGenerateResult(false, 'failed', '', [], '供应商未返回任务ID', $data);
            }
            return new PersonReplacementGenerateResult(true, $this->normalizeStatus($this->extractStatus($data) ?: 'running'), $taskId, [], '', $data, $this->extractUsage($data), true);
        } catch (\Throwable $e) {
            return new PersonReplacementGenerateResult(false, 'failed', '', [], $this->friendlyError($e->getMessage()));
        }
    }

    public function query(string $taskId, int $tenantId, int $userId): PersonReplacementGenerateResult
    {
        try {
            $config = $this->resolveConfig();
            ...
            if ($status === 'failed') {
                return new PersonReplacementGenerateResult(false, 'failed', $taskId, [], $this->extractError($data) ?: '动作替换任务失败', $data, $this->extractUsage($data));
            }
            if ($status !== 'success') {
                return new PersonReplacementGenerateResult(true, 'running', $taskId, [], '', $data, $this->extractUsage($data), true);
            }
            if (empty($videoUrls)) {
                return new PersonReplacementGenerateResult(false, 'failed', $taskId, [], '供应商未返回视频', $data, $this->extractUsage($data));
            }
            $videos = [];
            ...
            return new PersonReplacementGenerateResult(true, 'success', $taskId, $videos, '', $data, $this->extractUsage($data));
        } catch (\Throwable $e) {
            return new PersonReplacementGenerateResult(false, 'failed', $taskId, [], $this->friendlyError($e->getMessage()));
        }
    }
