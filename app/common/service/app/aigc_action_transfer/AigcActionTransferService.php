...
class AigcActionTransferService
{
    public const APP_CODE = 'aigc_action_transfer';
    public const UPSTREAM_APP_CODE = 'action_transfer';
    ...
    {
        $row = AigcActionTransferConfig::where('tenant_id', $tenantId)->findOrEmpty();
    ...
    public static function saveConfig(int $tenantId, array $params): void
    {
        ...
        if ($row->isEmpty()) {
            ...
            AigcActionTransferConfig::create($data);
            return;
        }
        $row->save($data);
    }
    ...
    public static function generate(int $tenantId, int $userId, array $params): array
    {
        ...
        $task = AigcActionTransferTask::create([
            'tenant_id' => $tenantId,
            'user_id' => $userId,
            'provider_task_id' => '',
        ...
        $provider = new XhadminActionTransferProvider();
        $result = $provider->submit($prepared['provider_payload']);
        if (!$result->success || $result->taskId === '') {
            $task->save([
                'status' => 'failed',
                'error' => $result->error ?: '动作迁移提交失败',
                'provider_response' => $result->raw,
                'finish_time' => time(),
                'update_time' => time(),
            ]);
            throw new Exception($result->error ?: '动作迁移提交失败');
        }

        Db::startTrans();
        try {
            $locked = AigcActionTransferTask::where('tenant_id', $tenantId)->where('id', (int)$task['id'])->lock(true)->findOrEmpty();
            if ($locked->isEmpty()) {
                throw new Exception('任务不存在');
            }
            PointService::consumeBusinessAmountsInCurrentTransaction($tenantId, $userId, (float)$estimate['tenant_cost_points'], (float)$estimate['user_charge_points'], self::sourceSn((int)$task['id']), '动作迁移消费', [
    ...
    public static function taskLists(int $tenantId, int $userId = 0, array $params = []): array
    {
        ...
        $query = AigcActionTransferTask::alias('t')
    ...
    public static function taskDetail(int $tenantId, int $taskId, int $userId = 0): array
    {
        ...
        $query = AigcActionTransferTask::where(['tenant_id' => $tenantId, 'id' => $taskId])->where('delete_time', 0);
        if ($userId > 0) {
    ...
    {
        $task = AigcActionTransferTask::where(['tenant_id' => $tenantId, 'id' => $taskId])->where('delete_time', 0)->findOrEmpty();
        if ($task->isEmpty()) {
            throw new Exception('任务不存在');
        }
    ...
    {
        $query = AigcActionTransferTask::where(['tenant_id' => $tenantId, 'id' => $taskId]);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
        $task = $query->findOrEmpty();
        ...
        AigcActionTransferResult::where(['tenant_id' => $tenantId, 'task_id' => $taskId])->update(['delete_time' => time(), 'update_time' => time()]);
    }

    public static function deleteResult(int $tenantId, int $resultId, int $userId = 0): void
    {
        $query = AigcActionTransferResult::where(['tenant_id' => $tenantId, 'id' => $resultId]);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
    ...
    private static function assertAvailable(int $tenantId, int $userId = 0): void
    {
        if (AppAccessService::assertTenantCanUse($tenantId, self::APP_CODE, $userId) !== null) {
            throw new Exception('动作迁移应用未开通或未上架');
        }
        $config = self::config($tenantId);
        if ((int)($config['status'] ?? 1) !== 1) {
            throw new Exception('动作迁移应用已停用');
        }
    }
    ...
    private static function preparePayload(int $tenantId, array $params, bool $requireFiles): array
    {
        ...
        if ($requireFiles) {
            ...
            self::assertOnlineUrl($videoUrl, '参考视频');
        }
        $providerPayload = [
    ...
    {
        $query = AigcActionTransferTask::where('tenant_id', $tenantId)->where('delete_time', 0)->whereIn('status', ['pending', 'running']);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
        if ($taskId > 0) {
    ...
    private static function syncTask(AigcActionTransferTask $task): void
    {
        $taskId = trim((string)$task['provider_task_id']);
        if ($taskId === '') {
            return;
        }
        $result = (new XhadminActionTransferProvider())->query($taskId, (int)$task['tenant_id'], (int)$task['user_id']);
        if ($result->pending) {
            $task->save([
        ...
        if (!$result->success || $result->status === 'failed') {
            self::markFailed($task, $result->error ?: '动作迁移任务失败', $result->raw, $result->usage);
            return;
        }
        self::finishTask($task, $result);
    }

    private static function finishTask(AigcActionTransferTask $task, ActionTransferGenerateResult $result): void
    {
        Db::startTrans();
        try {
            $locked = AigcActionTransferTask::where('tenant_id', (int)$task['tenant_id'])->where('id', (int)$task['id'])->lock(true)->findOrEmpty();
            if ($locked->isEmpty() || (string)$locked['status'] === 'success') {
                Db::commit();
                return;
            }
            $existing = AigcActionTransferResult::where('tenant_id', (int)$locked['tenant_id'])->where('task_id', (int)$locked['id'])->where('delete_time', 0)->findOrEmpty();
            if ($existing->isEmpty()) {
                ...
                AigcActionTransferResult::create([
    ...
    private static function markFailed(AigcActionTransferTask $task, string $error, array $raw = [], array $usage = []): void
    {
        Db::startTrans();
        try {
            $locked = AigcActionTransferTask::where('tenant_id', (int)$task['tenant_id'])->where('id', (int)$task['id'])->lock(true)->findOrEmpty();
            ...
            if ((string)$locked['billing_status'] === 'deducted') {
                PointService::refundBusinessAmountsInCurrentTransaction((int)$locked['tenant_id'], (int)$locked['user_id'], (float)$locked['tenant_cost_points'], (float)$locked['user_charge_points'], self::sourceSn((int)$locked['id']) . '-refund', '动作迁移失败退回', [
                    'app_code' => self::APP_CODE,
                    'task_id' => (int)$locked['id'],
                ]);
    ...
    private static function appendResults(int $tenantId, int $userId, array $rows): array
    {
        ...
        $map = [];
        if ($taskIds) {
            $query = AigcActionTransferResult::where('tenant_id', $tenantId)->where('delete_time', 0)->whereIn('task_id', $taskIds)->order('id', 'asc');
            if ($userId > 0) {
                $query->where('user_id', $userId);
            }
    ...
    {
        AigcActionTransferConfig::create([
            'tenant_id' => $tenantId,
            'status' => $data['status'],
