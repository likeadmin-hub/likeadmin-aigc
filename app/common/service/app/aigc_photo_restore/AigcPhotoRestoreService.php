...
namespace app\common\service\app\aigc_photo_restore;
...
class AigcPhotoRestoreService
{
    public const APP_CODE = 'aigc_photo_restore';
    public const IMAGE_APP_CODE = 'aigc_image';

    private const DEFAULT_PROMPT_TEMPLATE = '基于用户上传的老照片执行{restore_type_name}。尽量保持人物身份、五官、服饰、场景结构和历史质感真实自然，修复破损、模糊、噪点、划痕和褪色问题，输出清晰、干净、自然的高质量照片。{type_prompt}{user_prompt}';
    private const DEFAULT_NEGATIVE_PROMPT = '五官变形，身份变化，过度磨皮，塑料感，色彩失真，新增文字，水印，低清晰度，过度锐化，背景异常';
    private const DEFAULT_PRICE_PACKAGE_NAMES = ['标准修复', '高清修复'];

    private const DEFAULT_TYPES = [
        [
            'code' => 'repair',
            'name' => '照片修复',
            'description' => '修复破损、模糊、噪点和划痕。',
            'prompt' => '重点修复照片破损、模糊、噪点、划痕和低清晰度问题，保留原始黑白或已有色彩。',
            'sort' => 30,
        ],
        [
            'code' => 'colorize',
            'name' => '照片上色',
            'description' => '为黑白或褪色照片补充自然色彩。',
            'prompt' => '重点为黑白或褪色照片补充自然可信的色彩，肤色、服饰、场景光线应符合真实年代照片质感。',
            'sort' => 20,
        ],
        [
            'code' => 'repair_colorize',
            'name' => '照片修复+上色',
            'description' => '同时完成清晰修复和自然上色。',
            'prompt' => '同时完成照片破损修复、清晰度增强和自然上色，保持人物身份与历史照片真实感。',
            'sort' => 10,
        ],
    ];
    ...
    public static function saveConfig(int $tenantId, array $params): void
    {
        ...
        if ($row->isEmpty()) {
            ...
            AigcPhotoRestoreConfig::create($data);
            return;
        }
        $row->save($data);
    }

    public static function restoreTypeLists(int $tenantId, array $params = []): array
    {
        self::ensureDefaultTypes($tenantId);
        $query = AigcPhotoRestoreType::where('tenant_id', $tenantId)
    ...
    public static function saveRestoreType(int $tenantId, array $params): array
    {
        self::ensureDefaultTypes($tenantId);
        $code = self::normalizeTypeCode((string)($params['code'] ?? ''));
        if ($code === '') {
            $id = (int)($params['id'] ?? 0);
            $code = (string)AigcPhotoRestoreType::where(['tenant_id' => $tenantId, 'id' => $id])->value('code');
        }
        if (!self::isBuiltInType($code)) {
            throw new Exception('修复类型不支持新增或删除');
        }
        $row = AigcPhotoRestoreType::where(['tenant_id' => $tenantId, 'code' => $code])->findOrEmpty();
        if ($row->isEmpty()) {
            throw new Exception('修复类型不存在');
        }
    ...
    public static function setRestoreTypeStatus(int $tenantId, string $code, int $status): void
    {
        self::ensureDefaultTypes($tenantId);
        $code = self::normalizeTypeCode($code);
        if (!self::isBuiltInType($code)) {
            throw new Exception('修复类型不存在');
        }
        AigcPhotoRestoreType::where(['tenant_id' => $tenantId, 'code' => $code])
    ...
    public static function generate(int $tenantId, int $userId, array $params): array
    {
        ...
        if ($imageTaskId <= 0) {
            throw new Exception('老照片修复任务创建失败');
        }
    ...
    public static function taskLists(int $tenantId, int $userId = 0, array $params = []): array
    {
        ...
        $query = AigcPhotoRestoreTask::alias('t')
        ...
        $restoreType = self::normalizeTypeCode((string)($params['restore_type'] ?? ''));
        if ($restoreType !== '') {
            $query->where('t.restore_type', $restoreType);
        }
    ...
    public static function taskDetail(int $tenantId, int $taskId, int $userId = 0): array
    {
        ...
        $query = AigcPhotoRestoreTask::where(['tenant_id' => $tenantId, 'id' => $taskId])->where('delete_time', 0);
        if ($userId > 0) {
    ...
    {
        $task = AigcPhotoRestoreTask::where(['tenant_id' => $tenantId, 'id' => $taskId])->where('delete_time', 0)->findOrEmpty();
        if ($task->isEmpty()) {
            throw new Exception('任务不存在');
        }
    ...
    {
        $query = AigcPhotoRestoreTask::where(['tenant_id' => $tenantId, 'id' => $taskId]);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
        $task = $query->findOrEmpty();
        ...
        AigcPhotoRestoreResult::where(['tenant_id' => $tenantId, 'task_id' => $taskId])->update(['delete_time' => time()]);
    }

    public static function deleteResult(int $tenantId, int $resultId, int $userId = 0): void
    {
        $query = AigcPhotoRestoreResult::where(['tenant_id' => $tenantId, 'id' => $resultId]);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
    ...
    private static function assertAvailable(int $tenantId): void
    {
        if (AppAccessService::assertTenantCanUse($tenantId, self::APP_CODE) !== null) {
            throw new Exception('老照片修复应用未开通或未上架');
        }
        ...
        if ((int)($config['status'] ?? 1) !== 1) {
            throw new Exception('老照片修复应用已停用');
        }
    }
    ...
    private static function prepareGeneratePayload(int $tenantId, array $params, bool $requireImage): array
    {
        ...
        $typeCode = self::normalizeTypeCode((string)($params['restore_type'] ?? 'repair'));
        $restoreType = self::restoreType($tenantId, $typeCode, $requireImage);
        ...
        $imagePayload = [
            'prompt' => self::renderPrompt((string)$config['prompt_template'], [
                'restore_type_name' => (string)($restoreType['name'] ?? '照片修复'),
                'type_prompt' => (string)($restoreType['prompt'] ?? ''),
                'user_prompt' => trim((string)($params['prompt'] ?? $params['user_prompt'] ?? '')),
            ]),
    ...
    private static function upsertTaskFromImageTask(int $tenantId, int $userId, int $imageTaskId, array $prepared, array $estimate): AigcPhotoRestoreTask
    {
        ...
        $row = AigcPhotoRestoreTask::where(['tenant_id' => $tenantId, 'image_task_id' => $imageTaskId])->findOrEmpty();
        $restoreType = $prepared['restore_type'];
        $data = [
            'tenant_id' => $tenantId,
            'user_id' => $userId,
            'image_task_id' => $imageTaskId,
            ...
            'restore_type' => (string)$restoreType['code'],
            'restore_type_name' => (string)$restoreType['name'],
            'size_key' => $prepared['size_key'],
        ...
        if ($row->isEmpty()) {
            $data['delete_time'] = 0;
            $data['create_time'] = time();
            return AigcPhotoRestoreTask::create($data);
        }
        $row->save($data);
        return $row;
    }

    private static function syncTaskFromImageTask(AigcPhotoRestoreTask $task): void
    {
    ...
    private static function syncResultsFromImageTask(AigcPhotoRestoreTask $task): void
    {
        ...
        foreach (self::taskImageIds($task->toArray()) as $imageTaskId) {
            ...
            foreach (($imageTask['results'] ?? []) as $result) {
                ...
                $exists = AigcPhotoRestoreResult::where(['tenant_id' => $tenantId, 'image_result_id' => $imageResultId])->findOrEmpty();
                if (!$exists->isEmpty()) {
                    continue;
                }
                AigcPhotoRestoreResult::create([
                    'tenant_id' => $tenantId,
                    'task_id' => (int)$task['id'],
                    'image_task_id' => $imageTaskId,
    ...
    {
        $query = AigcPhotoRestoreTask::where('tenant_id', $tenantId)->where('delete_time', 0);
        if ($userId > 0) {
            $query->where('user_id', $userId);
        }
        if ($taskId > 0) {
    ...
    private static function appendTaskResults(int $tenantId, int $userId, array $rows): array
    {
        ...
        $resultMap = [];
        if ($taskIds) {
            $query = AigcPhotoRestoreResult::where('tenant_id', $tenantId)
                ->where('delete_time', 0)
    ...
    private static function restoreType(int $tenantId, string $code, bool $requireEnabled = false): array
    {
        self::ensureDefaultTypes($tenantId);
        $row = AigcPhotoRestoreType::where(['tenant_id' => $tenantId, 'code' => $code])->findOrEmpty();
        if ($row->isEmpty()) {
            throw new Exception('请选择修复类型');
        }
        $data = $row->toArray();
        if ($requireEnabled && (int)($data['status'] ?? 1) !== 1) {
            throw new Exception('修复类型已停用');
        }
        return $data;
    }
    ...
    private static function ensureDefaultTypes(int $tenantId): void
    {
        ...
        foreach (self::DEFAULT_TYPES as $type) {
            $row = AigcPhotoRestoreType::where(['tenant_id' => $tenantId, 'code' => $type['code']])->findOrEmpty();
            if (!$row->isEmpty()) {
                continue;
            }
            AigcPhotoRestoreType::create(array_merge($type, [
                'tenant_id' => $tenantId,
                'status' => 1,
                'create_time' => $time,
                'update_time' => $time,
    ...
    private static function unitPrice(array $priceConfig, string $channel, string $quality, string $ratio): float
    {
        ...
        throw new Exception('请先配置老照片修复价格');
    }
    ...
    private static function saveConfigSnapshot(int $tenantId, array $data, AigcPhotoRestoreConfig $row): void
    {
        $payload = [
            'tenant_id' => $tenantId,
        ...
        if ($row->isEmpty()) {
            ...
            AigcPhotoRestoreConfig::create($payload);
            return;
        }
        $row->save($payload);
    }
    ...
    private static function renderPrompt(string $template, array $data): string
    {
        ...
        $typePrompt = trim((string)($data['type_prompt'] ?? ''));
        return trim(strtr(self::normalizeTemplate($template), [
            '{restore_type_name}' => (string)($data['restore_type_name'] ?: '照片修复'),
            '{type_prompt}' => $typePrompt !== '' ? '修复要求：' . $typePrompt . '。' : '',
    ...
    private static function isBuiltInType(string $code): bool
    {
        return in_array($code, array_column(self::DEFAULT_TYPES, 'code'), true);
    }
