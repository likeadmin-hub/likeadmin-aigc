...
class AigcDigitalHumanChannelService
{
    ...
    public static function estimate(int $tenantId, array $params): array
    {
        ...
        $duration = max(1, (int)($params['duration'] ?? $params['audio_duration'] ?? $params['seconds'] ?? 1));
        $estimate = AigcDigitalHumanPricingService::estimateGenerate($tenantId, $duration, (string)$resolved['channel']['code']);
    ...
    public static function resolveSelection(int $tenantId, array $params): array
    {
        ...
        throw new Exception('数字人通道不可用');
    }

    public static function platformLists(): array
    {
        $channels = AigcDigitalHumanChannel::where('tenant_id', 0)->order(['sort' => 'desc', 'id' => 'asc'])->select()->toArray();
        $specs = AigcDigitalHumanChannelSpec::where('tenant_id', 0)->order(['sort' => 'desc', 'id' => 'asc'])->select()->toArray();
        $grouped = [];
    ...
    public static function savePlatform(array $params): void
    {
        ...
        $id = (int)($params['id'] ?? 0);
        $where = $id > 0 ? ['tenant_id' => 0, 'id' => $id] : ['tenant_id' => 0, 'code' => $code];
        $row = AigcDigitalHumanChannel::where($where)->findOrEmpty();
        if ($row->isEmpty()) {
            throw new Exception('内置通道不存在');
        }
        $originCode = (string)$row['code'];
        if ($code !== $originCode && !AigcDigitalHumanChannel::where(['tenant_id' => 0, 'code' => $code])->findOrEmpty()->isEmpty()) {
            throw new Exception('通道编码已存在');
        }
        $data = [
            'tenant_id' => 0,
            'code' => $code,
            ...
            'model' => trim((string)($params['model'] ?? 'mock-digital-human')) ?: 'mock-digital-human',
            'max_reference_images' => (int)($row['max_reference_images'] ?? self::DEFAULT_REFERENCE_LIMIT),
            'config_json' => self::normalizePlatformChannelConfig(array_merge($params['config_json'] ?? [], [
        ...
        try {
            $row->save($data);
            if ($code !== $originCode) {
                self::renamePlatformChannelCode($originCode, $code);
            }
            Db::commit();
        } catch (\Throwable $e) {
            Db::rollback();
    ...
    public static function savePlatformSpec(array $params): void
    {
        ...
        self::updateBuiltInRow(AigcDigitalHumanChannelSpec::class, [
            'tenant_id' => 0,
            'channel_code' => $channelCode,
            'quality' => $quality,
            'ratio' => $ratio,
    ...
    {
        $model = (($params['type'] ?? '') === 'spec') ? AigcDigitalHumanChannelSpec::class : AigcDigitalHumanChannel::class;
    ...
    public static function saveTenant(int $tenantId, array $params): void
    {
        ...
        self::saveRow(AigcDigitalHumanChannel::class, ['tenant_id' => $tenantId, 'code' => $code], $data);
    }
    ...
    public static function saveTenantSpec(int $tenantId, array $params): void
    {
        ...
        $quality = strtolower(trim((string)($params['quality'] ?? '')));
        $ratio = trim((string)($params['ratio'] ?? ''));
        $platform = AigcDigitalHumanChannelSpec::where([
            'tenant_id' => 0,
            'channel_code' => $channelCode,
            'quality' => $quality,
            'ratio' => $ratio,
        ...
        $data = [
            ...
            'tenant_unit_price' => (string)($platform['tenant_unit_price'] ?? $platform['platform_unit_cost']),
            'billing_unit' => (string)($platform['billing_unit'] ?? 'second'),
            'provider_params_json' => [],
            'status' => (int)($params['status'] ?? 1),
            'sort' => (int)($params['sort'] ?? $platform['sort']),
            'update_time' => time(),
        ];
        self::saveRow(AigcDigitalHumanChannelSpec::class, [
            'tenant_id' => $tenantId,
            'channel_code' => $channelCode,
            'quality' => $quality,
            'ratio' => $ratio,
    ...
    private static function effectiveChannels(int $tenantId, bool $onlyEnabled): array
    {
        ...
        foreach ($platformChannels as $platformChannel) {
            ...
            $channel = [
                ...
                'description' => self::channelDescription($platformChannel),
            ...
            foreach ($specsByChannel[$platformChannel['code']] ?? [] as $platformSpec) {
                ...
                $spec = [
                    ...
                    'billing_unit' => (string)($platformSpec['billing_unit'] ?? 'second'),
    ...
    {
        $channel = AigcDigitalHumanChannel::where(['tenant_id' => 0, 'code' => $code])->findOrEmpty();
        if ($channel->isEmpty()) {
    ...
    private static function normalizePlatformChannelConfig($value, $origin): array
    {
        ...
        unset($data['poll_interval'], $data['poll_attempts']);
    ...
    private static function channelDescription(array $channel): string
    {
        $config = self::normalizeJson($channel['config_json'] ?? []);
        ...
        $defaults = [
            'master' => '高质量数字人视频模型，适合正式口播和营销内容',
            'all' => '通用数字人视频模型，适合产品介绍和知识讲解',
            'free' => '轻量体验模型，适合快速试用和短文案预览',
        ];
        return $defaults[$code] ?? '标准数字人视频模型';
    }

    private static function renamePlatformChannelCode(string $originCode, string $newCode): void
    {
        AigcDigitalHumanChannel::where(['tenant_id' => 0, 'code' => $originCode])->update(['code' => $newCode, 'update_time' => time()]);
        AigcDigitalHumanChannel::where('tenant_id', '>', 0)->where('code', $originCode)->update(['code' => $newCode, 'update_time' => time()]);
        AigcDigitalHumanChannelSpec::where(['tenant_id' => 0, 'channel_code' => $originCode])->update(['channel_code' => $newCode, 'update_time' => time()]);
        AigcDigitalHumanChannelSpec::where('tenant_id', '>', 0)->where('channel_code', $originCode)->update(['channel_code' => $newCode, 'update_time' => time()]);
        AigcDigitalHumanPricingService::renameModelCode($originCode, $newCode);
    }
        ...
        foreach ($channels as &$channel) {
            ...
            foreach ($channel['qualities'] as &$quality) {
                foreach ($quality['ratios'] as &$ratio) {
                    unset($ratio['provider_params_json'], $ratio['platform_unit_cost'], $ratio['tenant_unit_price'], $ratio['billing_unit']);
                }
            }
        }
        return $channels;
