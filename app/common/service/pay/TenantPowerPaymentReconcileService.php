...
class TenantPowerPaymentReconcileService
{
    public static function inspect(TenantPowerOrder $order): array
    {
    ...
    public static function reconcile(TenantPowerOrder $order): bool
    {
        if ((int)$order['pay_status'] === PayEnum::ISPAID) {
            return true;
        }
        ...
        $result = PayNotifyLogic::handle(TenantPowerMallService::FROM, (string)$order['order_sn'], [
    ...
    private static function inspectWechat(array $order): array
    {
        ...
        $response = (new WeChatPayService($terminal, null, true))->checkPay($paySn);
