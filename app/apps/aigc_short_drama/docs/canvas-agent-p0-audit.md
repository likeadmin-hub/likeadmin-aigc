...
- A01：`ConversationImages` 已按显式选中顺序冻结并解析多达 4 张已授权图片。隔离 `p2_worker.php` 最新 **99 PASS / 0 FAIL**，新增双图顺序、无额外媒体任务断言；此前的真实单图 Qwen 证据仍有效。**真实双图比较质量**未再次发起付费调用，保持 NOT_RUN，不能以这项隔离通路代替。
...
- A10：`ConversationTextContext` 从历史 user 消息中仅提取明确且长度受限的画风、比例、时长，作为当前 user-role JSON 的 `known_creation_constraints`；提示模型除非用户修改，不重复询问。没有把这些文本提升为 system 指令、工具参数或生成任务。隔离 Worker 验证后续请求收到 `国风水墨 / 9:16 / 15 秒`。模型实际回答是否遵循仍需真实对话样本，故该项为**实现与输入契约 PASS、真实语义质量 NOT_RUN**。
