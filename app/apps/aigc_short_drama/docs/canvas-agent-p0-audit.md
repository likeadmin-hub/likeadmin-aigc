...
> **当前验收运行方式（2026-09-22，替代此前隔离环境设定）**：所有后续短剧画布 Agent 验收仅复用现有本地 Baota 服务、`x_cn` 数据库、现有 Worker 与本地 PC 浏览器。不得创建或恢复独立 Docker 网络、MySQL、测试镜像、测试容器、测试数据库或额外进程守护；测试也不得自动迁移本地数据库。历史章节中关于 `short-drama-agent-test` 的记录仅保留为已发生的历史证据，不再是可执行前置条件。当前命令、夹具边界与清理规则以 `tests/agent/README.md` 为准。
...
## 62. P3 M09 用户豁免与 P4 附件草稿可靠性（2026-09-22）

用户明确决定不再为本地 tenant 1 配置或改造真实私有存储桶，故 M09 的“真实私有签名 URL HTTP 刷新”标记为 **WAIVED（用户豁免，不计 PASS）**。现有公开存储不被改写为私有，既有 M09 服务实现和隔离签名测试证据仍保留；该豁免不影响其余 P4—P6 的实施，但后续若产品启用私有存储，必须重新执行真实对象的签名刷新验收。

web `dbf5b64` 在 Composer 上传链补齐 P4 的首个可靠性切片：附件从草稿移除后，异步文本读取、上传响应和图片素材登记每一步都会重新确认该附件仍在当前草稿；移除后的迟到结果不再回写状态、错误或 `assetId`，也不会成为随后对话提交的引用。PDF、DOC、DOCX 不再走模糊的预览提交路径，而是在提交前明确提示“解析尚未接入，请移除或改用 TXT/Markdown；未创建任务”；既有 TXT/Markdown 和 100KB 限制不变。
...
## 65. P4 文档解析中的附件移除语义（本地 PC 回归通过，2026-09-22）

用户明确确认：文档解析已经提交后，不实现取消或退款承诺。上游 `file_qa` 当前没有可由本地可靠确认的取消结果；将输入框中的“移除”直接映射成取消会错误声称上游任务、实际用量或账本已经撤销。

- 已实现：PDF/DOC/DOCX 处于“解析中”时，附件的移除按钮保留可点击状态，但只提示“文档解析中，暂不能移除”，附件、解析轮询和已提交的上游任务均保持不变。
- 已实现：待确认、已完成、失败的文档，以及其他素材，仍维持原有移除语义；本次没有新增取消接口、退款逻辑、物理文件删除或 Worker。
...
## 67. P4 Agent 自动/手动建节点与生成语义（本地行为回归通过，2026-09-22）

用户确认 Agent 对话的自动模式不设置积分上限；该选择不放宽模型能力、租户归属、图版本、请求幂等或视频报价确认。为避免普通咨询被错误解释为付费生成，本轮没有把自然语言直接当作执行指令。

- 已实现：Agent 只有在用户明确要求生成且模型在答复中附带受限 `canvas-actions` 提案时，才允许创建最多 4 个文本、图片或视频节点。提案不接受模型 ID、价格、URL、资产 ID、身份字段或任意工具调用；服务端从冻结的租户模型偏好补入节点模型，并使用 GraphService 的同一版本化写路径创建节点与当前选中来源的 reference 连线。
- 已实现：手动模式仅创建 `idle` 节点；页面重载权威画布后，由用户在节点上点击生成。自动模式标记受控节点，页面重载权威画布后复用既有 node run/request-key/账本入口自动提交文本和图片。视频仍停在节点和一次既有报价确认，不能把 Agent 输出当作跳过报价的永久授权。
- 已实现：畸形、重复或超范围的结构化提案作为已知无效模型输出失败，不写图、不创建媒体任务、不重试；普通不带提案的对话仍保持纯文本、无图变更。
- PASS：现有本地 Baota 的 `p4_agent_generation_mode.php` 共 **14 PASS / 0 FAIL**，覆盖手动/自动冻结模式、节点创建、服务端模型快照、选中节点连线、自动标记与畸形提案拒绝。`p2_worker.php` 全量回归保持 PASS，确认普通对话不会创建节点或媒体任务。PC `short-drama-canvas-composer.test.cjs` 共 **19 PASS / 0 FAIL**，包含 SFC 编译、权威重载和仅自动提交合格节点的合同断言。
- NOT_RUN：真实浏览器中 Agent 模型实际输出结构化提案后的自动文本/图片提交、视频报价确认，以及真实 Provider/账本结算；本轮不调用付费模型、没有新增环境、数据库、容器或 Worker。现有浏览器套件依赖仍缺 Playwright，不能把组件回归标为端到端通过。

本项完成的是 P4 自动/手动动作的受控实现与本地回归证据；它不使 P4 或后续 P5/P6 自动放行，剩余门槛继续按真实能力和逐项验收状态记录。

## 68. P4 自动节点的 Worker 恢复提交（本地行为回归通过，2026-09-22）

此前自动模式在 Agent 回复完成后由当前浏览器调用画布节点提交接口。若页面在该时间窗关闭、刷新，或浏览器与 Worker 同时尝试恢复，可能没有后续提交，或因两个不同请求键形成两份生成意图。

- 已实现：Agent 在同一受锁图写入中为自动文本、图片节点分配服务端 `agent_auto_run_id` 与稳定请求键；前端重载后优先复用该键，不能自行生成另一份浏览器 UUID。
- 已实现：既有 `short-drama:canvas-agent-worker` 每轮在会话队列后扫描本租户已保存的自动节点；只处理仍为 `idle` 的文本/图片。它仍调用画布既有 `submitIdempotent` / generation intent / 计费入口，不新增进程、队列或表；已排队、成功、失败、删除的节点不重提。
- 已实现：媒体引用只读取上游已成功且已资产化的 source node；未完成或没有稳定资产 ID 的依赖保持等待，不会把空 URL 发送给 Provider。视频始终不进入自动恢复器，仍须在节点上取得并确认实时报价。
- PASS：现有 Baota 本地库运行 `p4_agent_auto_recovery.php`，共 **11 PASS / 0 FAIL**；覆盖 Agent 回复先持久化、稳定请求键、仅文本/图片可恢复、已排队不重提和视频/终态排除。`p4_agent_generation_mode.php` 原有 **14 PASS / 0 FAIL** 与 `p2_worker.php` 回归保持 PASS。PC `short-drama-canvas-composer.test.cjs` 共 **20 PASS / 0 FAIL**，包括服务端自动键的浏览器复用和 Vue SFC 编译。
- NOT_RUN：本轮没有重启常驻 Worker、没有让真实 Agent 或 Provider 触发自动节点、没有进行浏览器端关闭/刷新后的真实自动生成，也没有调用付费模型、结算或报价确认。因此不得将这一回归结果标作真实 Provider/计费通过。

此切片补上 P4 自动模式的恢复和同键幂等基础；完整 R01–R10 的 DAG 调度、局部重试、外部回调故障演练，以及其余 P4 门槛仍未放行。

## 69. P4 Agent 同轮依赖调度与局部失败隔离（本地行为回归通过，2026-09-22）

此前 Agent 自动节点只能等待已有媒体来源可用：同一轮创建的后续节点没有受限的依赖表达，且上游失败时下游会无限等待。现有 Agent Worker、画布行版本及 generation intent 已足以承载该能力，因此没有新增队列、表、进程、容器或验收环境。

- 已实现：结构化 `canvas-actions` 每个节点可选用本轮临时 `key` 与 `depends_on` 声明最多 3 个**已经在前面出现**的前序 key。服务端在解析层和 GraphService 写入层各自严格验证；拒绝未来引用、循环、重复 key、重复依赖、节点 ID、素材 ID、模型和其他越权字段。
- 已实现：被认可的依赖写为权威画布中的 `reference / agent_dependency` 边。文本、图片、视频的既有四节点连接矩阵仍强制执行；不支持的边不会写入图。
- 已实现：既有 Agent Worker 扫描自动节点时先读权威图。前序未成功时只让直接下游等待；前序 `failed/canceled` 时只把该仍为 `idle` 的自动下游标为失败并给出“前序节点生成失败，未提交此依赖节点”。无依赖或不依赖失败节点的兄弟继续可调度，且不会生成 Provider 请求、run 或扣费。手动节点提交也会使用同一服务端门槛，在前序成功前不创建 generation intent、run 或 Provider 请求；已资产化的前序媒体会被服务端加入下游受管引用。提交入口会再次做同一判断，以覆盖 Worker 与浏览器的竞争窗口。
- PASS：本机既有 Baota、`x_cn` 本地库运行 `p4_agent_dependency_scheduler.php` **13 PASS / 0 FAIL**。覆盖提案与依赖边的原子写入、自动/手动等待、手动门槛零 run、局部失败、独立节点继续、持久化失败提示及两种非法依赖在写图前拒绝。夹具只使用并精确清理 tenant 91034/user 92034 的临时画布记录。
- PASS：既有 `p4_agent_generation_mode.php` **14 PASS / 0 FAIL**、`p4_agent_auto_recovery.php` **11 PASS / 0 FAIL**，以及 `p2_worker.php` 全量回归均保持通过。
- NOT_RUN：真实 Provider 提交、实际常驻 Worker 重启恢复、浏览器渲染/点击、视频报价确认、真实账本结算与多任务并发压力。本切片没有发起付费调用；也不能把本地行为回归扩大为 R01/R02 的完整真实 Provider 验收。

这一步完成 P4 的 DAG 表达、就绪判断和局部故障隔离基础。R01 的真实多视频并发、R02 的真实上游失败重试、R03–R10 的进程/回调/账本故障注入和其余 P4 门槛仍按真实能力继续验收。

## 70. P4 R08 停止后的外部状态诚实边界（本地行为回归通过，2026-09-22）

当前 Agent 对话 Run 已有独立于画布节点的提交边界；本轮在本机既有 Baota 与本地 `x_cn` 运行 `p2_stop.php`，核对其能否作为 P4 R08 的前置保护，而不是将本地停止错误地写成上游已取消。

- PASS：tenant/user/canvas/thread 的归属检查拒绝跨范围停止；排队、处理和 preflight 阶段停止均终止本地 Run、释放会话且不调用 Provider。
- PASS：已跨越提交边界的停止只记录一次 `run.stop_requested` 并保持 `needs_reconciliation`；返回值明确 `cancellation_confirmed=false`，迟到回复被保留为核实证据但不会发布为成功。
- PASS：重复停止返回稳定结果，已成功对话不被停止改写，禁用 Agent 后仍可停止已排队的本地 Run；整个验收不创建媒体任务。
...
## 72. P6 PC 基础画布回归（本地 develop，2026-09-22）

使用桌面自带 Node 运行时在 web 本地 `develop` 逐项执行：连接 **10 PASS**、持久化 **3 PASS**、分组 **5 PASS**、控制 **5 PASS**，合计 **23 PASS / 0 FAIL**。没有启动浏览器桥、Provider、Worker 或新环境。
...
## 70. 租户端独立 Agent 工作流管理入口（2026-09-22）

- 按最新产品要求，将 Agent 设置从基础配置移到与 Skill 管理并列的“短剧画布 Agent”页面；页面内设创作工作流列表和八阶段编辑弹窗。仅修改租户端，未增加平台端页面。
- 配置读取增加 `canvas_agent_workflows` 集合，目前仅注册短剧成片制作。后续工作流需配套注册执行逻辑，不将列表扩展能力等同于任意流程设计器。
- Agent 开关、工作流配置、基础配置各自保存时保留其他配置；未知工作流键被拒绝。已开始的对话继续使用冻结版本，视频手动和音频禁用规则不变。
- **PASS**：两个 Vue 页面 script/template 编译；本地 tenant 1 事务内验证三类配置互不覆盖及未知工作流拒绝，事务回滚恢复配置。
- **PASS**：现有宝塔运行态精确同步 tenant 1 新菜单，重复执行仅保留一条菜单；未批量同步或删除其他菜单，未修改平台菜单。
- **PASS**：已登录租户端真实浏览器显示与 Skill 管理并列的入口、工作流列表及八阶段编辑弹窗；展开选择器看到已发布 Skill 版本，保存未改动的工作流后弹窗关闭，重新加载仍显示已启用的八阶段工作流和原 Agent 开关。第 69 节登录阻碍已解除。
- **NOT_RUN**：本轮未重新执行真实 Provider 和完整创作链路，此次管理入口验收不代表 P5/P6 全部放行。
- 源码：server `88ca3876f`，web `e2ae3c4`；均已提交 feature/short-drama-optimization 并合并本地 develop 后验证。未新建运行环境、Docker、数据库或 Worker，未部署、发布、推送或调用付费生成。

## 2026-09-23 · Agent 画布产物与真实输入连线收敛

参考已可见的 Seko 创作流程，并以本应用现有短剧生成输入链路为准，新工作流版本 `2026-09-23.3` 只把故事设定与全剧/分集大纲、当前单集剧本、主体图、主体三视图、场景图、分镜图、手动生成的分镜视频，以及禁用生成的音频规划节点投影到画布。画风/资产规格和逐镜视频提示词仍由模型输出、用户确认并存入对话工作流状态，但不再为它们创建画布文本节点；关键道具先写入镜头提示词，不默认生成独立道具节点。当前单集之外的逐集批处理**未实现**，不把一次生成误称为覆盖用户选择的全部集数。

- `reference_keys` 只绑定确实被选为生成输入的既有素材；文本规划被服务端并入相应媒体节点的冻结提示词，不画无意义连线。用户选中的节点不再自动连向整批新节点。主体三视图必须依赖对应主体图；分镜图依赖本镜头场景，并按需选择主体图/三视图；视频节点必须引用对应分镜图，不以视频间串联代替素材输入。图边仍在现有节点报价和提交入口解析为实际 Provider 请求素材。
- 新对话冻结新版合同；已经开始的旧版对话保持其原三文本节点和依赖语义，不删除或隐藏用户已有画布内容。当前 canvas 20 的旧节点没有自动清理，以免破坏历史任务、素材或连线。
- 图片计划的 hash 现在包含被显式引用的既有画布素材及其内容版本、资产/投影身份。确认前替换引用图会返回 `WORKFLOW_PLAN_SOURCE_CHANGED`，不沿用旧确认。没有新增计费或媒体任务系统。
...
## 2026-09-23 · 短剧采集问题按已有内容生成

- **实现**：语义路由或显式 `/short-drama` 的同一文本模型回合，提取本轮描述、已解析且获授权的文档、可供视觉模型读取的图片中的高置信设定，并拟定缺失项的上下文问题。只允许既有七项采集字段；文档/图片来源标签必须有本轮冻结材料支持。未识别出可靠信息时继续用原问题卡兜底。
- **实现**：提取结果先显示可编辑核对卡，不自动写入确认槽位。用户可以修订、删除或忽略候选；确认后只追问尚缺的信息，已确认字段不会在后续回合被重复打开。来源材料和模型输出都不能直接创建节点、媒体任务或改变计费。旧版进行中对话保留原合同。
...
## 2026-09-23 · P5 正式写回产品决策（仅合同确认，未实现）

用户确认：画布剧本或分镜应用到正式短剧项目时，先展示源与目标的差异，由用户逐项确认写入指定剧集或镜头；若预览后源版本、目标版本或绑定目标发生变化，必须拒绝覆盖并重新预览。既有自由画布生成任务、素材、账本与项目归属不得因绑定或预览而搬迁。现有 `ShortDramaCanvasBindingService` 仅解析并记录当前目标；`ShortDramaStoryDraft` 和 `saveStoryboard` 有各自的正式写入规则，尚未形成画布产物到这些目标的统一、可确认映射。因此 P5 D04–D08/D10 仍为 **NOT_RUN / 未放行**；本条不把产品决策当实现或验收。

## 2026-09-23 · P5 写回来源发现（只读基础，不代表正式写回）

- **实现**：新增只读 `canvas/writebackSources`。它先按现有 tenant/user/canvas 归属读取绑定，再只列出当前画布中未删除的 Agent 剧本阶段文本产物；返回节点 ID、artifact、标题、`content_revision`、内容 SHA-256 和画布版本，不返回正文，也不接受客户端伪造的项目或制作项目 ID。普通手工文本、非剧本阶段文本、已删除节点均不成为正式写回候选。接口沿用短剧用户场景登录和画布权限。
- **PASS / 本地 develop**：`p5_binding.php` **12 PASS / 0 FAIL**，含来源过滤、绑定版本、内容版本/摘要、跨租户读取拒绝和既有自由画布任务/资产不变；`p2_http.php` 原 Agent HTTP 权限及 SSE 回归全部通过；新服务 PHP 语法和 API schema JSON 解析通过。测试仅用现有 Baota、现有本地 `x_cn` 回滚夹具，无 Provider、积分或额外运行环境。
- **NOT_RUN / P5 D04–D08、D10**：该接口只提供来源清单，不解析自由正文到正式结构化字段，也没有源/目标差异预览、逐项确认、目标 CAS 或正式写入。用户对旧自由正文映射与新结构化字段的选择尚在确认中；在明确前不把自由正文直接覆盖项目字段。真实浏览器该入口也未验收。
- **提交**：server 功能 `8c88a25e6` 已 `--no-ff` 合入本地 `develop` 后测试；本文档证据另行提交。web 未改。未推送、部署、发布或新建服务。

## 2026-09-23 · P5 多集故事草稿逐字段预览（只读）

- **实现**：新增 `canvas/previewStoryWriteback`，仅接受已绑定故事项目（非剧集制作项目）中的 Agent `story_setting` 文本节点和明确选择的 `title/type_judgement/core_theme/story_outline` 目标字段。目标必须是当前成功任务的、尚未分配剧集的 `story_outline_v2` 多集故事草稿；其他阶段或单集正式项目不会被这个路径误写。接口返回源/目标实际正文与内容摘要，以及绑定版本、源内容版本、正式项目版本和草稿版本共同形成的预览指纹；明确 `can_apply=false`，不保存任何正式内容。
- **PASS / 本地 develop**：`p5_binding.php` **19 PASS / 0 FAIL**，含错误剧集绑定拒绝、真实源/目标差异、源文变化、目标文变化、正式项目版本变化、切走再绑定使旧预览失效、跨租户拒绝和自由画布历史任务/资产不变。所有夹具在现有 Baota 与本地 `x_cn` 的事务中回滚；PHP 语法通过。共享画布控制器 `p0_controller.php` 回归通过，未修改现有 `ShortDramaStoryDraft::save`、媒体任务、账本或 Worker。
- **NOT_RUN / P5 D04–D08、D10 完整门槛**：预览指纹不是已生效的写入许可；尚未实现锁内重读、逐字段确认、正式草稿保存和浏览器差异界面。当前预览只适用多集故事草稿，不适用旧自由正文自动拆字段、分集大纲、单集剧本或分镜镜头。新 HTTP 路由的本地注册和浏览器点击也未验收，不能以直接服务测试代替。旧节点是否允许整体写入指定字段仍等用户决定；在明确前不执行正式覆盖。
- **提交**：server `2325ea797`、`f6eedb032` 均已 `--no-ff` 合入本地 `develop` 后完成行为测试；本文档另行提交。web 本轮未改，未调用 Provider、积分或新增运行环境，也未推送/部署/发布。

## 2026-09-23 · P5 结构化故事/分集逐项写回（局部验收）

- **实现**：新开启的短剧工作流版本 `2026-09-23.5` 要求剧本阶段的 `story_setting` 与 `episode_script` 输出明确、受限的 `formal_fields`；Worker 解析、Graph 二次校验并把字段和正文摘要作为服务端元数据持久化。既有冻结版本维持旧合同。旧自由正文、缺少结构化字段或正文后来被手工修改的节点不能被猜测拆字段写回。
- **实现**：新增故事草稿与分集大纲的预览/确认 API。只允许当前用户绑定的多集项目、当前成功的 `story_outline_v2` 任务及尚可编辑的阶段；来源集号必须对应正式大纲集号。预览展示单个字段的实际源/目标内容；确认后锁内重读画布、绑定、项目、任务并核对版本指纹，复用 `ShortDramaStoryDraft::save`，只写被选字段。重复确认通过任务内有界回执返回原结果，不再推进草稿版本。PC Agent 面板提供绑定项目、选择来源与目标字段、预览差异、逐项确认入口。
- **PASS / 现有本地 `x_cn`**：`p5_binding.php` 回滚式行为测试通过（故事及分集精确字段、目标变化冲突拒绝、源/绑定/项目版本变化、确认门槛、幂等重放、跨租户拒绝、自由画布历史不迁移）。`p5_compact_canvas_projection.php` 通过，覆盖新字段保留与缺字段模型输出拒绝；冻结旧版 `p5_multi_skill_workflow.php` 回归通过；新服务与控制器 PHP 语法通过，PC ESLint `--quiet` 0 error。测试未调用真实 Provider 或积分。
...
## 2026-09-23 · P5 分镜写回真实浏览器闭环（临时数据已清理）

- **PASS / 本地真实浏览器 D06–D08、D10 分镜路径**：在现有 Baota `x_cn`、tenant 1、当前登录用户下创建有唯一 `[P5B]` 标记的临时画布与正式剧集夹具；浏览器访问临时画布，绑定临时故事项目的第 1 集，选择 Agent 分镜图来源和指定正式镜头，看到真实来源“雨夜办公室里，主角低头查看一盘旧录音带。”与目标“验收前的画面描述”的差异及提示词重建说明。点击逐镜确认后页面显示“所选字段已写回正式项目”；刷新页面重新预览，正式目标已与来源一致。
- **PASS / 数据核对**：指定镜头 `visual_description` 为新内容，原 `composition=远景` 保留，`image_prompt` 与 `video_prompt` 均非空且由既有规则重建，正式项目生成版本已推进；该制作项目新增媒体生成任务数为 0。画布积分显示在操作前后均为 10633.98，未调用 Provider。关闭验收标签后使用精确 canvas ID 133 清理夹具；画布、父/制作项目、剧集、脚本任务、分镜、版本、Agent Run、绑定等相关记录复查均为 0。未修改既有用户项目或创建新环境、容器、Worker。
- **PASS / 回归**：在本地 `develop` 通过 `p5_binding.php` 37 项服务行为检查；临时夹具脚本 PHP 语法通过。夹具脚本首次因闭包缺少 `$stamp` 变量失败，事务回滚且无残留；修复后重新执行完整浏览器闭环。
- **NOT_RUN / P5 其余门槛**：上述浏览器 PASS 仅覆盖正式镜头 `visual_description` 路径，不代表故事/分集逐字段浏览器确认、单集正式项目文本写回或新版真实文本 Provider 输出结构化字段已经端到端通过。P5 整体仍未放行，P6 状态不变。

## 2026-09-23 · P5 故事与分集逐项写回真实浏览器闭环

- **PASS / 故事字段浏览器闭环**：在现有本地 `x_cn` 的 tenant 1 当前登录用户下，以唯一 `[P5T]` 标记创建临时故事项目和画布，绑定故事项目，选择 `story_setting` 来源及“故事大纲”。页面真实展示来源“Agent 生成的新故事梗概”和目标“旧故事梗概”；点击确认后显示成功，刷新并重新预览时正式目标已是来源内容。只用了结构化 `workflow_formal_fields`，没有把自由正文猜测拆字段。
- **PASS / 分集字段浏览器闭环**：将同一临时项目精确切换到可编辑分集大纲阶段，选择 `episode_script` 来源、第 1 集“分集大纲”，页面展示真实差异；点击确认、刷新并重新预览后，第 1 集正式目标为“Agent 生成的第一集大纲”。数据库核对第 2 集仍为“旧第二集大纲”，`_story_draft.version=1`。前后积分显示均为 10633.98，未创建媒体任务或调用 Provider。
- **PASS / 清理**：关闭验收标签后以精确 canvas ID 136 清理临时画布、项目、脚本任务和绑定；逐表复查残留均为 0。夹具脚本 `p5_text_browser_fixture.php` 仅允许 `SHORT_DRAMA_AGENT_TEST=local-existing`、现有 `x_cn` 和有唯一标记的本机 tenant 1 数据；PHP 语法通过。未修改用户既有项目、没有新建环境、容器或 Worker。
- **NOT_RUN / P5 整体仍未放行**：单集正式项目的故事/剧本保存合同与多集草稿不同，尚未实现并验收；新工作流版本通过真实 Provider 输出结构化字段后一路写回的端到端样本也未执行。上述三条已有浏览器 PASS（故事、分集、分镜）不等于整个 P5 阶段放行。

## 2026-09-23 · P5 旧预览拒绝覆盖的真实浏览器验收

- **PASS / D05 浏览器并发保护**：在当前本地 Baota `x_cn` 的 tenant 1、当前登录用户下创建唯一 `[P5T]` 临时画布 138 和正式项目 1261。浏览器绑定项目后预览 `story_setting` → `story_outline`，看见来源“Agent 生成的新故事梗概”和目标“旧故事梗概”。预览保持打开时，验收夹具仅将这个临时项目的正式目标改成“另一编辑者刚保存的新故事梗概”。点击旧确认后，页面显示“版本已变化，请重新预览”，确认卡被撤销；重新预览显示刚修改的正式目标，未被旧来源覆盖。未将服务级 CAS 断言代替本项浏览器行为。
- **PASS / 清理与回归**：关闭临时标签后精确清理画布 138、项目 1261、任务 `p5_text_20260923115643-af839f` 与绑定；只读复查四类记录均为 0。夹具仅新增受唯一前缀、阶段和旧内容约束的 `change_target` 动作，未接触现存用户项目。现有本地 `develop` 上 PHP 语法通过，`p5_binding.php` **37 PASS / 0 FAIL**；无 Provider、积分任务、新环境、容器或 Worker。
- **NOT_RUN / P5 整体**：单集正式项目文本写回与新版工作流真实文本 Provider 输出 `formal_fields` 后的浏览器端到端仍未验收。单集没有分集任务行，不能直接复用多集 `ShortDramaStoryDraft`；先核准其精确写入字段和正文/分镜保留规则，再实现适配，不能把这次并发保护 PASS 视为 P5 完成。

## 2026-09-23 · 用户取消“写回正式项目”功能

- 用户明确要求直接移除，不仅隐藏入口。本节覆盖上方关于正式项目绑定、逐项预览和写回的后续实施计划；上方测试结果仅保留为历史证据，不再是当前产品能力或待放行门槛。
- 删除 PC 正式项目写回入口/差异交互及其 API 调用，删除画布控制器的九个专用动作、绑定/写回服务、API 声明与相关专用夹具。新安装 SQL 不再创建正式项目绑定表；升级 SQL 仅移除这九条 API 注册。已有安装的绑定表与历史记录保留，不执行删表或改动任何正式项目、画布、任务、素材和账本数据。
- 新建工作流版本不再要求模型输出仅供正式写回的 `formal_fields`，也不再向新节点投影这些字段；已提交的旧模型响应若包含它们，解析时忽略，避免把已付费响应错误判为失败。现有节点的历史元数据不清除。
- “写回原文本节点”是画布内部文本改写确认，与本次移除的“写回正式项目”不是同一功能，继续保留。四节点画布、Agent 对话及工作流、普通短剧生成与媒体任务链路不因本次变更关闭。
- **PASS / 本地 develop**：PC 画布对话/任务组件测试 32 PASS / 0 FAIL，含 Vue SFC 编译与“正式项目入口不存在、原文本节点写回保留”的断言；PHP 变更文件语法通过。`p6_local_compatibility.php` 确认九个控制器动作/API 声明均不存在、画布普通保存与报价动作仍在、旧画布 JSON 完整可读；`p5_compact_canvas_projection.php` 验证新剧本输出无需旧正式字段、旧已提交回复的字段可忽略、新节点不投影它们，主体/三视图/分镜/视频连线与提交策略通过；`p5_multi_skill_workflow.php`、`p0_controller.php`、`p2_http.php` 回归通过。上述测试未调用真实 Provider 或付费生成，脚本报告的其他 P6/Provider 项仍为 NOT_RUN。
- **PASS / 当前本地 `x_cn`**：删除前精确核对只存在九条本功能的 `scene=user` API 注册；在现有宝塔数据库事务内执行本次升级 SQL 后九条均消失。未删绑定表、既有画布、正式项目或用户数据。新开本地浏览器画布 97 验证“应用到正式项目”和逐项写回面板均不存在，聊天记录入口仍在；未提交任务或更改画布。
- **提交**：server `b52229163`，web `3f03138`；均已 `--no-ff` 合入各自本地 `develop`，本节证据追加提交另记。未推送、部署、发布或新建环境。正式项目写回已从当前产品范围取消，并非 P5 未验收待补项。

## 2026-09-23 · 进行中工作流逐轮意图分流

- **根因与修复**：旧路由只在工作流未激活时进行语义分类；已有工作流会在入队时无条件 `continue`，甚至把闲聊当作当前阶段输入。现将进行中工作流的每轮回复限定为一次模型调用内的受控意图与阶段产物联合 JSON；只有高置信 `continue` 且阶段产物通过原有解析、计划与图谱校验后，才更新阶段。普通聊天、独立图片/视频请求、新短剧需求及歧义询问不改工作流或画布；显式 `/Skill` 仍优先。待确认卡期间可聊天，但不能绕过确认。
- **PASS / 现有本地环境**：在现有 Baota 容器、`x_cn` 中，`p5_multi_skill_workflow.php` 验证闲聊→同一美术阶段续接、自动图片阶段仍进入既有报价确认、无关图片需求不产出节点、待确认期间闲聊保持原卡、模型在闲聊中夹带工作流产物被拒；`p2_send.php` 验证真实发送入口冻结逐轮路由且不提前推进状态，以及 `/Skill` 覆盖；`p2_worker.php`、`p2_http.php` 回归通过。全部使用合成租户夹具、回滚或精确清理，未新增环境、容器、Worker、迁移或真实付费调用。
- **NOT_RUN**：真实付费文本 Provider 对不同意图的稳定分类质量与浏览器实际交互尚未以本变更单独验收；不能把模拟 Provider 的 PASS 宣称为真实模型效果。

## 2026-09-23 · 单集 Agent 全阶段真实付费闭环与中途卡住修复

- **范围**：仅本机现有 Baota、tenant 1 的合成测试画布 `177`、对话 `398`；使用当前已启用的 Qwen 文本和真实图片 Provider，单次提交均在用户允许的 2000 积分上限内。前端验收页为 `http://localhost:3000/ai/short-drama/canvas?canvas_id=177&tenant_id=1`，已在当前浏览器打开并显示最终工作流卡。没有新建 Docker、数据库、运行环境或独立 Worker，未提交视频、音频任务。
- **发现并修复的卡点 1 / 真实模型结构**：剧本阶段的模型回复把继续创作标识放在 `skill_key` 等建议字段，旧解析器将已完成的付费文本结果误判为不合合同；美术/视频规划回复又可能带有空引用或无效计划连线，造成阶段报错。现在只在受限意图、当前阶段及原有图谱验证全部成立时接纳建议字段，并对可选空引用做规范化，不允许模型绕过确认或提交任意画布 JSON。旧失败 Run `601/666/667/679` 保留为审计历史，不删除、不宣称它们成功。
- **发现并修复的卡点 2 / 前端重载循环**：对话组件重新挂载会重放历史画布动作，父组件过去不论版本是否已安装都重载画布，导致再次卸载/挂载、反复请求会话并停在“正在恢复对话”。现在只有服务端动作的 `graph_revision` 新于当前版本才重载；自动模式仍在重放时检查未提交的待生成节点，保留崩溃恢复。现有浏览器重新打开后会话、卡片和画布稳定展示，不再每秒循环读取会话。PC 对话/持久化相关 Node 回归 **31 PASS / 0 FAIL**。
- **PASS / 真实文本 Provider**：`599` 显式 `/short-drama` 进入创作采集，`600` 的无关蒙太奇问题得到普通聊天回复且不推进阶段；修复后 `665` 剧本、`703` 美术、`704` 主体资产、`705` 分镜规划、`706` 视频规划、`707` 视频节点、`708` 音频规划均为 `success`。`706` 的非计费规划确认在真实浏览器点击后进入下一阶段；最终页面工作流卡为“已完成 8/8”。完成后 `726` 回答无关数学问题，工作流仍停在完成态、图谱未被普通聊天改写。
- **PASS / 多个真实图片任务**：画布生成 Run `64–69` 对应主体主图、由主图作为输入的主体三视图、旧书店场景、三个不同分镜图，全部 `success/100` 并显示结果。镜头图提交的冻结引用分别按需包含场景、主体和三视图；重复提交 Run `65` 的原请求键与原载荷返回同一任务，没有再生成或重复扣费。改变载荷却复用请求键正确返回 `IDEMPOTENCY_CONFLICT`，不是一次新任务。当前图谱为 12 节点、14 条边；仅 2 个剧本文本节点，未把美术规划或视频规划全文堆成画布节点。
- **PASS / 视频和音频边界**：三个分镜视频节点一次插入并连接对应分镜/必要主体，均为 `idle`，画布视频 Run 为 **0**，没有自动报价或上游提交；音频规划节点为禁用状态，尝试通过既有生成入口提交被 `WORKFLOW_AUDIO_GENERATION_UNAVAILABLE` 拒绝，音频 Run 为 **0**。现有本地 `develop` 的 `p5_multi_skill_workflow.php` 对完成卡、阶段权限和原有工作流行为回归 **0 FAIL**。
- **NOT_RUN / 不冒充完全放行**：本次真实付费闭环使用**手动图片模式、单集、三个分镜**；自动模式“整套图片计划报价确认后依赖提交”、多集逐集批处理、真实视频 Provider 和音频生成均没有在此轮执行。视频仍按产品规则必须用户逐节点确认，音频当前明确不开放。此前 P5/P6 其他尚未放行的门槛不因这个单集样本自动变为 PASS。

## 2026-09-23 · Agent 视频实际参考与原短剧创作提示词共源

- **修复**：原短剧视频请求以当前分镜图为首帧，优先引用该镜头绑定角色的三视图（无三视图才用主体主图），再引用绑定场景图。Agent 新建视频节点现在由服务端依据分镜图真实入边补足这一组素材；每个视频仅允许一个对应分镜图，仍只创建待用户手动生成节点。视频适配器不再把同一已授权素材的存储 URI 和签名 URL 当成两张图片。
- **提示词**：新版本工作流 `2026-09-23.7` 开始时冻结原短剧 `ShortDramaPromptWorkspace` 的租户有效配置；剧本、美术、主体/三视图、场景/分镜、视频规划分别读取原 `ShortDramaPromptDocuments` 对应创作规则。Agent 的受限 JSON/图谱/计费指令仍独立，普通闲聊不应用创作规则；旧已冻结会话不被悄悄改写。tenant 1 当前原提示词配置为 documents 修订 1；画布 177 的会话 398 是已完成的 `.6` 版本。
- **PASS / 现有本地环境**：`p5_compact_canvas_projection.php` 覆盖原提示词工作区覆盖、租户文档模式、配置不外泄、视频三张真实参考、旧待生成视频修复幂等；`p3_asset_version_reference.php` 覆盖私有素材身份及避免重复签名 URL；`p5_multi_skill_workflow.php`、`p3_reference_assets.php` 回归通过。画布 177 精确核对用户 1/租户 1、视频节点 9/10/11 均 idle 且无视频 Run 后，受限修复将图谱版本 64→65；三个视频现在分别引用 `[937,935,936]`、`[938,935,936]`、`[939,935,936]` 对应的分镜、角色三视图、场景素材，依赖状态均 ready。未提交视频、音频或任何真实付费调用。
- **FAIL / 原有非本变更项**：更宽范围的 `p0_generation.php idempotent` 在模拟音频生成断言处失败；此前的文本、图片、视频、幂等断言通过。本变更没有开放音频；不能把该整套脚本记为 PASS。
- **NOT_RUN**：真实视频 Provider、浏览器刷新后的连线视觉复核、新版 `.7` 的真实付费文本输出质量、自动模式多镜头批处理，仍未验收。未新增环境、Docker 或 Worker，未部署、推送或发布。

## 2026-09-23 · Agent 媒体提交复用正式短剧模板

- **实现**：从工作流 `.8` 开始，新建主体主图、三视图、场景、分镜图和分镜视频节点记录所属 Agent Run。报价与提交前根据该 Run 的租户/用户/画布归属读取冻结的原短剧提示词工作区，经正式短剧同一个 `applyConfiguredGenerationPromptTemplate` 入口包装节点当前原始提示词。保留原流程对 legacy/workspace/documents 模式及自定义文档的既有优先级，不复制第二套模板；旧冻结会话与普通手工节点不追溯改写。视频依旧先报价并由用户确认，音频仍禁用。
- **PASS / 现有 Baota `x_cn`**：新增回滚式 `p5_shared_submission_templates.php` 验证三视图、分镜图、分镜视频映射到正式短剧模板、跨租户快照拒绝、视频报价与提交提示词完全一致、同请求键只调用一次模拟 Provider、手工修改提示词重新报价；`p5_compact_canvas_projection.php` 验证新图片/视频节点绑定冻结 Run，全部原有图谱/引用断言通过。`p3_quote_confirmation.php`、`p4_agent_generation_mode.php` 全部通过；修改文件 PHP 语法检查通过。测试夹具在事务内回滚，未改 tenant 1 真实配置或项目数据。
- **NOT_RUN**：真实付费图片/视频 Provider 提交、浏览器交互与现有 `.6/.7` 会话的重新创作没有在本次执行。画布 177 原已完成的 `.6` 会话不会被静默改写；要用新规则须启动新 `.8` 工作流。未新建环境、容器、独立 Worker、迁移，未推送或部署。
