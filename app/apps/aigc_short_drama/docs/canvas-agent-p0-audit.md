...
- **PASS / 现有本地环境**：在现有 Baota 容器、`x_cn` 中，`p5_multi_skill_workflow.php` 验证闲聊→同一美术阶段续接、无关图片需求不产出节点、待确认期间闲聊保持原卡、模型在闲聊中夹带工作流产物被拒；`p2_send.php` 验证真实发送入口冻结逐轮路由且不提前推进状态，以及 `/Skill` 覆盖；`p2_worker.php`、`p2_http.php` 回归通过。全部使用合成租户夹具、回滚或精确清理，未新增环境、容器、Worker、迁移或真实付费调用。
