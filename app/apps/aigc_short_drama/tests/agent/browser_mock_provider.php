...
if (!defined('SHORT_DRAMA_BROWSER_MOCK_PROVIDER')) throw new RuntimeException('Mock provider requires isolated router');
use think\facade\Db;
...
final class BrowserMockProvider {
    public static function submit(string $type,int $tenant,int $user,array $input): array {
        ...
        if (($input[$type==='text'?'model_code':'channel']??'')!=='isolated-browser-mock') throw new RuntimeException('Only explicit synthetic model is allowed');
        ...
        PointService::consumeBusinessAmountsInCurrentTransaction($tenant,$user,1,2,'browser-mock-'.$receipt,'Isolated browser mock');
        if ($type==='text') return ['content'=>'Synthetic browser result','provider'=>'isolated-mock'];
        ...
        Db::name($table.'_result')->insert(['tenant_id'=>$tenant,'user_id'=>$user,'task_id'=>$id,$column=>'https://fixtures.invalid/browser-'.$receipt,'storage_scope'=>'tenant','storage_engine'=>'local']);
