...
try {
    ...
    agentCheck(
        $savedPreferences['code']===1
        && (string)($savedPreferences['data']['preferences']['reasoning_model']??'')===(string)$product
        && (string)($savedPreferences['data']['preferences']['generation_mode']??'')==='auto'
        && (int)($savedPreferences['data']['revision']??0)===1,
        'HTTP persists validated default model with CAS revision'
    );
    ...
    agentCheck(agentHttp('threads','GET',['canvas_id'=>$canvas])['code']===1,'disabled Agent retains owned thread list');
    agentCheck(agentHttp('messages','GET',$args)['code']===1,'disabled Agent retains owned message history');
    agentCheck(agentHttp('events','GET',$args)['code']===1,'disabled Agent retains owned event history');
    agentCheck(agentHttp('run','GET',$runArgs)['code']===1,'disabled Agent retains owned run status');
    agentCheck(agentHttp('workflow','GET',$workflowArgs)['code']===1,'disabled Agent retains frozen workflow state');
    agentCheck(agentHttp('messages','GET',$args,'isolated-agent-other')['code']!==1,'disabled Agent history still rejects foreign user');
    agentCheck(agentHttp('createThread','POST',['canvas_id'=>$canvas,'request_key'=>'disabled-new'])['msg']==='CANVAS_AGENT_DISABLED','disabled Agent rejects new conversations');
    agentCheck(agentHttp('send','POST',array_replace($send,['request_key'=>'disabled-send']))['msg']==='CANVAS_AGENT_DISABLED','disabled Agent rejects new sends');
    agentCheck(agentHttp('answerWorkflow','POST',$workflowArgs+['expected_revision'=>1,'slot'=>'genre','value'=>'悬疑'])['msg']==='CANVAS_AGENT_DISABLED','disabled Agent rejects workflow answers');
    agentCheck(agentHttp('confirmWorkflowPlan','POST',$workflowArgs+['expected_revision'=>1,'plan_hash'=>str_repeat('a',64)])['msg']==='CANVAS_AGENT_DISABLED','disabled Agent rejects image plan confirmation');
    agentCheck(agentHttp('confirmWorkflowStage','POST',$workflowArgs+['expected_revision'=>1])['msg']==='CANVAS_AGENT_DISABLED','disabled Agent rejects stage confirmation');
