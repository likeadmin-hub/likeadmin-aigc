...
try {
    ...
    $node=['id'=>1,'type'=>'video','x'=>10,'y'=>20,'metadata'=>['video_url'=>'fixture/video.mp4','poster_url'=>'fixture/old.jpg','canvasRunId'=>123]];
    ...
    $poster->invoke(null,['tenant_id'=>91001,'user_id'=>92001,'canvas_id'=>$id,'node_id'=>'1','video_uri'=>'fixture/video.mp4'],['uri'=>'fixture/new.jpg','storage_scope'=>'platform','storage_engine'=>'local','storage_domain'=>'']);
    $doc=Canvas::current(91001,92001,$id);
    agentCheck($doc['graph_revision']===2 && $doc['nodes'][0]['metadata']['poster_uri']==='fixture/new.jpg','poster projection advances same graph revision');
    ...
    agentCheck($doc['graph_revision']===3 && $doc['nodes'][0]['x']===777 && $doc['nodes'][0]['metadata']['poster_uri']==='fixture/new.jpg','explicit current-base movement keeps background poster');
    $project=new ReflectionMethod(Canvas::class,'projectVideoRunToCanvas');$project->setAccessible(true);
    $run=['id'=>123,'tenant_id'=>91001,'user_id'=>92001,'canvas_id'=>$id,'node_id'=>'1'];
    $result=['url'=>'fixture/new-video.mp4','poster_url'=>'fixture/final.jpg'];
    ...
    $run['id']=122;$project->invoke(null,$run,['url'=>'fixture/late-video.mp4','poster_url'=>'fixture/late.jpg']);
    $doc=Canvas::current(91001,92001,$id);
    agentCheck($doc['graph_revision']===4 && $doc['nodes'][0]['metadata']['video_url']==='fixture/new-video.mp4','old run does not change current video or graph revision');
    ...
    $doc=Graph::patch(91001,92001,$id,['request_key'=>'integrated-move','expected_revision'=>5,'operations'=>[['op'=>'move_nodes','nodes'=>[['id'=>1,'x'=>888,'expected_layout_revision'=>0]]]]]);
