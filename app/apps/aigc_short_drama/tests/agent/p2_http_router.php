...
if (!preg_match('#^/api/app\.aigc_short_drama\.canvas_agent/(threads|preferences|savePreferences|createThread|renameThread|deleteThread|messages|events|send|stop|run|stream|workflow|answerWorkflow|confirmWorkflowPlan|confirmWorkflowStage)$#D',(string)$path)) {http_response_code(404);exit;}
