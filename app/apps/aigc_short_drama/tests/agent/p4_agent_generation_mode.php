...
final class AgentGenerationModeProvider implements ConversationProviderInterface
{
    ...
    public function generate(int $tenant,int $user,array $request): array
    {
        ...
        return ['content'=>'已准备好画布内容。<canvas-actions>{"nodes":[{"type":"text","title":"剧情梗概","prompt":"雨夜重逢的三幕剧情梗概"},{"type":"image","title":"雨夜街景","prompt":"电影感雨夜街景，霓虹灯"}]}</canvas-actions>','tool_calls'=>[]];
    }
}
...
try {
    ...
    foreach (['manual','auto'] as $mode) {
        ...
        agentCheck(count($message['canvas_actions']['nodes']??[])===2,$mode.' action projects two opaque created node IDs');
        $nodes=json_decode((string)Db::name('aigc_short_drama_canvas')->where('id',$canvas)->value('nodes_json'),true);
        $created=array_slice($nodes,-2);
