...
class WorkbenchLogic extends BaseLogic
{
     ...
     */
    public static function index(int $tenantId = 0): array
    {
        $tenantId = self::tenantId($tenantId);
        $appStats = self::appStats($tenantId);
        $trend = self::trend($tenantId);
        $today = self::today($tenantId, $appStats);
     ...
     */
    public static function today(int $tenantId = 0, array $appStats = []): array
    {
        $tenantId = self::tenantId($tenantId);
        ...
        return [
            ...
            'tenant_point_balance' => self::round(Tenant::withoutGlobalScope()->where('id', $tenantId)->value('point_balance')),
            'user_total' => (int)User::where('tenant_id', $tenantId)->count(),
            'today_new_user' => (int)User::where('tenant_id', $tenantId)->where('create_time', '>=', $todayStart)->count(),
            'user_money_total' => self::round(User::where('tenant_id', $tenantId)->sum('user_money')),
            ...
            'membership_order_total' => self::paidOrderCount('membership_order', $tenantId),
            'today_membership_order' => self::paidOrderCount('membership_order', $tenantId, $todayStart),
            'recharge_order_total' => self::paidOrderCount('recharge_order', $tenantId),
            ...
            'order_num' => self::paidOrderCount('membership_order', $tenantId, $todayStart) + self::paidOrderCount('recharge_order', $tenantId, $todayStart),
            'order_sum' => self::paidOrderCount('membership_order', $tenantId) + self::paidOrderCount('recharge_order', $tenantId),
        ];
    }
    ...
    public static function summaryCards(array $today): array
    {
        return [
            ...
            [
                'title' => '任务成功率',
                'value' => $today['success_rate'],
                'unit' => '%',
                'sub_title' => '总任务',
                'sub_value' => $today['task_total'],
            ],
            [
                'title' => '运行中任务',
     ...
     * @author likeadmin
     */
    public static function trend(int $tenantId = 0): array
    {
        $tenantId = self::tenantId($tenantId);
        ...
        foreach (self::APP_DEFINITIONS as $app) {
            if (!empty($app['task_table'])) {
                self::mergeTaskTrend($trend, (string)$app['task_table'], $range, $tenantId);
            }
            if (!empty($app['billing_table'])) {
                self::mergeBillingTrend($trend, (string)$app['billing_table'], $range, $tenantId);
            }
            if (!empty($app['usage_table'])) {
                self::mergeUsageTrend($trend, (string)$app['usage_table'], $range, $tenantId);
            }
        }
    ...
    public static function appStats(int $tenantId = 0): array
    {
        $tenantId = self::tenantId($tenantId);
        $stats = [];
        foreach (self::APP_DEFINITIONS as $app) {
            $task = !empty($app['usage_table'])
                ? self::usageStats((string)$app['usage_table'], $tenantId)
                : self::taskStats((string)($app['task_table'] ?? ''), $tenantId);
            $billing = !empty($app['usage_table'])
                ? self::usageBillingStats((string)$app['usage_table'], $tenantId)
                : self::billingStats((string)($app['billing_table'] ?? ''), $tenantId);
    ...
    private static function taskStats(string $table, int $tenantId): array
    {
        $empty = [
            'task_total' => 0,
            'success_total' => 0,
        ...
        $todayStart = strtotime('today');
        $query = Db::name($table)->where('tenant_id', $tenantId);
    ...
    private static function usageStats(string $table, int $tenantId): array
    {
        $empty = [
            'task_total' => 0,
            'success_total' => 0,
        ...
        $todayStart = strtotime('today');
        $query = Db::name($table)->where('tenant_id', $tenantId)->where('billing_status', 'deducted');
    ...
    private static function billingStats(string $table, int $tenantId): array
    {
        $empty = [
    ...
    private static function usageBillingStats(string $table, int $tenantId): array
    {
        if ($table === '' || !self::tableExists($table)) {
            return [
                'tenant_cost_points' => 0,
    ...
    private static function mergeTaskTrend(array &$trend, string $table, array $range, int $tenantId): void
    {
    ...
    private static function mergeBillingTrend(array &$trend, string $table, array $range, int $tenantId): void
    {
        if (!self::tableExists($table)) {
            return;
        }
    ...
    private static function mergeUsageTrend(array &$trend, string $table, array $range, int $tenantId): void
    {
        if (!self::tableExists($table)) {
            return;
        }
    ...
    private static function paidOrderCount(string $table, int $tenantId, ?int $startTime = null): int
    {
        if (!self::tableExists($table)) {
            return 0;
        }
        $query = Db::name($table)->where('tenant_id', $tenantId)->where('pay_status', PayEnum::ISPAID);
        if ($startTime !== null) {
    ...
    private static function tenantId(int $tenantId): int
    {
        if ($tenantId > 0) {
            return $tenantId;
        }
        return (int)(request()->tenantId ?? request()->adminInfo['tenant_id'] ?? 0);
    }
