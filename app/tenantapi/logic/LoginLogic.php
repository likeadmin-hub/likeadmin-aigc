...
class LoginLogic extends BaseLogic
{
    ...
    public function sso(array $params)
    {
        $tenantId = (int)($params['tenant_id'] ?? request()->tenantId ?? 0);
        $ticket = (string)($params['sso_ticket'] ?? '');
        $terminal = (int)($params['terminal'] ?? 1);
        if (!$tenantId || $ticket === '') {
            throw new \Exception('免登录参数缺失');
        }
        return TenantSsoService::consume($ticket, $tenantId, $terminal);
    }
