...
(9327,{tenantId},9320,'C','创作任务','',19,'app.aigc_short_drama.generation_task/lists','creation-task','apps/aigc_short_drama/creation-task','','',0,1,0,'aigc_short_drama','app','aigc_short_drama_creation_task',0,1778000000,1778000000),
...
AND parent.tenant_id={tenantId} AND NOT EXISTS (SELECT 1 FROM `la_tenant_system_menu_{tenantSn}` existing WHERE existing.tenant_id=parent.tenant_id AND existing.app_code='aigc_short_drama' AND existing.source_menu_key='aigc_short_drama_agent');
