<template>
    ...
    <view class="recharge p-[20rpx]">
        <view class="bg-white rounded-[14rpx] p-[40rpx]">
            <view class="text-content">充值点数</view>
...
const { isLock, lockFn: rechargeLock } = useLockFn(async () => {
    ...
    if (!money.value) return uni.$u.toast('请输入充值点数')
    if (minNum == 0 && Number(money.value) == minNum) {
        return uni.$u.toast(`充值点数必须大于0`)
    }
    if (Number(money.value) < minNum) return uni.$u.toast(`最低充值点数${minNum}`)
