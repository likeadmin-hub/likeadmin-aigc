<template>
    <view class="page">
        ...
        >
            <view class="summary">
                <view>
                    <view class="summary__label">AIGC IMAGE GALLERY</view>
                    <view class="summary__title">历史图片</view>
                </view>
            ...
            <view v-if="lists.length" class="grid">
                <view v-for="item in lists" :key="item.id" class="card">
                    <view class="card__delete" @click.stop="handleDelete(item.id)">
                    ...
                    <view class="card__body">
                        ...
                        <view class="card__actions">
                            ...
                            <view
                                v-if="item.image_url"
                                class="action"
                                @click="saveImageToPhotosAlbum(item.image_url)"
                            >
            ...
            <view v-else class="empty">
                <view class="empty__icon">
                    <u-icon name="photo" color="rgba(255,255,255,0.74)" size="72"></u-icon>
                </view>
                <view class="empty__title">暂无作品</view>
                <view class="empty__desc">生成后的图片会保存在这里</view>
...
import { deleteAigcImageTask, getAigcImageResults } from '@/apps/aigc_image/api'
...
const getData = async () => {
    lists.value = await getAigcImageResults(status.value ? { status: status.value } : undefined)
}
...
const goCreate = () => {
    ...
    uni.navigateTo({ url: '/apps/aigc_image/pages/index/index' })
}

const reuseResult = (item: any) => {
    uni.setStorageSync('aigc_image_reuse', {
        prompt: item.prompt || '参考这张作品继续生成同风格图片',
        reference_images: item.image_uri ? [item.image_uri] : [],
        ratio: item.ratio || '1:1',
        quality: item.quality || '1k',
        quantity: 1,
        channel: item.channel || 'master'
    })
    const pages = getCurrentPages()
    const previousPage = pages[pages.length - 2] as any
    if (previousPage?.route === 'apps/aigc_image/pages/index/index') {
        uni.navigateBack()
        return
    }
    uni.navigateTo({ url: '/apps/aigc_image/pages/index/index' })
...
const previewImage = (url: string) => {
    if (!url) return
    uni.previewImage({
        urls: lists.value.map((item) => item.image_url).filter(Boolean),
        current: url
    })
}
...
const statusIcon = (value: string) => {
    return value === 'failed' ? 'close-circle' : value === 'success' ? 'photo' : 'clock'
}
