    ...
    <view class="page">
        ...
        <view class="topbar" :style="topbarStyle">
            <view class="nav-row" :style="navRowStyle">
                ...
                <view class="page-title" :style="pageTitleStyle">AIGC生图</view>
        ...
        <scroll-view class="content" scroll-y>
            ...
            <view v-if="qualities.length > 1" class="section">
                <view class="label-row">
                    ...
                    <view class="label-text">分辨率：</view>
                </view>
                <view class="segmented">
                    <view
            ...
            <view v-if="ratios.length > 1" class="section">
                <view class="label-row">
                    ...
                    <view class="label-text">图片比例：</view>
                </view>
            ...
            <view class="section">
                <view class="label-row label-row--between">
                    ...
                    <view class="section-link" @click="goResults">历史图片</view>
                </view>
                        ...
                        >
                            ...
                            <view class="example-info">
                                ...
                                <view class="example-meta"
                                    >{{ item.ratio }} · {{ item.quality.toUpperCase() }} ·
                                    {{ item.quantity }}张</view
                                >
            ...
            <view v-if="results.length" class="section">
                <view class="label-row label-row--between">
                    <view class="label-left">
                        <view class="label-icon">
                            <u-icon name="photo-fill" color="#ffffff" size="30"></u-icon>
                        </view>
                ...
                <view class="result-grid">
                    <view v-for="item in results.slice(0, 4)" :key="item.id" class="result-card">
                        <image
                            ...
                            :src="item.image_url"
                            mode="aspectFill"
                            @click="previewResult(item.image_url)"
                        />
...
const examples: ExampleItem[] = [
    {
        id: 1,
        title: '产品海报',
        prompt: '一只透明玻璃质感的智能耳机，悬浮在黑色镜面平台上，冷白色轮廓光，极简高端产品广告，细节锐利',
        ratio: '1:1',
        quality: '1k',
        quantity: 1,
        channel: 'master',
    ...
    },
    {
        id: 2,
        title: '电影人像',
        prompt: '夜晚街头的人像写真，霓虹反光，浅景深，电影感构图，真实皮肤质感，光影细腻',
        ratio: '3:4',
        quality: '2k',
        quantity: 2,
        channel: 'all',
    ...
    },
    {
        id: 3,
        title: '国潮插画',
        prompt: '国潮风山海主题插画，层叠云雾，金色线条装饰，东方神话氛围，画面丰富但主体清晰',
        ratio: '9:16',
        quality: '4k',
        quantity: 1,
        channel: 'master',
...
const channels = computed<ChannelOption[]>(() =>
    (optionConfig.value.channels || []).map((channel: any) => ({
        ...
        quantity_options: (
            channel.quantity_options ||
            optionConfig.value.quantity_options || [1, 2, 3, 4]
        )
...
const quantities = computed<number[]>(() => {
    const configured = (
        ...
        .map((item: any) => Number(item))
        .filter(Boolean)
    return Array.from(new Set([...configured, 1, 2, 3, 4])).sort((a, b) => a - b)
})
...
const getData = async () => {
    results.value = await getAigcImageResults({ status: 'success' })
    if (results.value.length && refreshTimer) {
        stopResultPolling()
    }
}
...
const applyReuseCache = () => {
    const cached = uni.getStorageSync('aigc_image_reuse')
    if (!cached) return
    form.prompt = cached.prompt || form.prompt || '参考这张作品继续生成同风格图片'
    ...
    syncSelection()
    uni.removeStorageSync('aigc_image_reuse')
}
...
const handleGenerate = async () => {
    ...
    try {
        const submitCount = Math.max(1, Math.min(Number(form.quantity) || 1, 4))
        ...
        await userStore.getUser()
        stopResultPolling()
        uni.navigateTo({ url: '/apps/aigc_image/pages/results/results' })
    } finally {
...
const previewResult = (url: string) => {
    uni.previewImage({
        urls: results.value.map((item) => item.image_url).filter(Boolean),
        current: url
    })
}

const reuseResult = (item: any) => {
    form.prompt = item.prompt || form.prompt || '参考最近作品继续生成同风格图片'
    form.reference_images = item.image_uri ? [item.image_uri] : []
...
const ensureMembershipAccess = async () => {
    try {
        const data = await getMembershipAppAccess({ app_code: 'aigc_image' })
