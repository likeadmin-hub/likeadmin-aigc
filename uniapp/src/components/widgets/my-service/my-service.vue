...
const showList = computed(() => {
    const list = props.content.data?.filter((item: any) => item.is_show == '1') || []
    const hasMembership = list.some(
        (item: any) => item.link?.path === '/packages/pages/membership/membership'
    )
    if (hasMembership) return list
    return [
        {
            name: '会员中心',
            image: '/static/images/icon/icon_user.png',
            link: { path: '/packages/pages/membership/membership' },
            is_show: '1'
        },
        ...list
    ]
