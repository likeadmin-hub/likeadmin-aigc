...
<script lang="ts" setup name="platform-aigc-digital-human-tenant-usage">
import { getAigcDigitalHumanTenantStat } from '@/apps/aigc_digital_human/api'
...
const getData = async () => {
    loading.value = true
    try {
        stat.value = await getAigcDigitalHumanTenantStat({ tenant_id: tenantId.value || 0 })
    } finally {
        loading.value = false
    }
}
