...
<script lang="ts" setup name="platform-aigc-image-channel">
import { getAigcImageChannels, saveAigcImageChannel, setAigcImageChannelStatus } from '@/apps/aigc_image/api'
...
const getLists = async () => {
    loading.value = true
    try {
        lists.value = await getAigcImageChannels()
    } finally {
        loading.value = false
    }
}
...
const handleSubmit = async () => {
    saving.value = true
    try {
        await saveAigcImageChannel(formData)
...
const handleStatus = async (row: any, status: number) => {
    ...
    try {
        await setAigcImageChannelStatus({ id: row.id, status })
        row.status = status
