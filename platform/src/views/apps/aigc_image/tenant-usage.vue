...
<script lang="ts" setup name="platform-aigc-image-tenant-usage">
import { getAigcImageTenantStat } from '@/apps/aigc_image/api'
...
const getData = async () => {
    loading.value = true
    try {
        stat.value = await getAigcImageTenantStat({ tenant_id: tenantId.value || 0 })
    } finally {
        loading.value = false
    }
}
