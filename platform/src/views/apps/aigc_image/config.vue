<template>
    <div>
        <el-card class="!border-none" shadow="never">
            <div class="text-lg font-medium">AIGC生图平台配置</div>
...
<script lang="ts" setup name="platform-aigc-image-config">
import { getAigcImagePlatformConfig, getAigcImageTenantStat, setAigcImagePlatformConfig } from '@/apps/aigc_image/api'
...
const getData = async () => {
    loading.value = true
    try {
        Object.assign(formData, await getAigcImagePlatformConfig())
        stat.value = await getAigcImageTenantStat()
    } finally {
        loading.value = false
    }
}
const handleSubmit = async () => {
    await setAigcImagePlatformConfig(formData)
    getData()
}
getData()
</script>
