        ...
        <el-card class="!border-none table-card" shadow="never">
            <el-table v-loading="pager.loading" size="large" :data="tableLists" height="100%">
                ...
                <el-table-column label="应用" min-width="110">
                    <template #default>智能剪辑</template>
                </el-table-column>
        ...
        <el-dialog v-model="editVisible" title="编辑通道" width="560px" destroy-on-close>
            <el-form label-width="110px" :model="formData">
                ...
                <el-form-item label="参考图上限">
                    <el-input-number v-model="formData.max_reference_images" :min="0" :max="0" class="w-full" disabled />
                </el-form-item>
...
<script lang="ts" setup name="platform-smart-clip-channel">
import { getSmartClipChannels, saveSmartClipChannel, setSmartClipChannelStatus } from '@/apps/smart_clip/api'
...
const normalizeForm = (row: any = {}) => ({
    code: row.code || '',
    name: row.name || '',
    provider: row.provider || 'xhadmin',
    model: row.model || 'smart_clip',
    max_reference_images: 0,
    config_json: {
...
const getLists = async () => {
    pager.loading = true
    try {
        lists.value = await getSmartClipChannels()
...
const handleSubmit = async () => {
    saving.value = true
    try {
        await saveSmartClipChannel(formData)
...
const handleStatus = async (row: any, status: number) => {
    ...
    try {
        await setSmartClipChannelStatus({ id: row.id, status })
        row.status = status
