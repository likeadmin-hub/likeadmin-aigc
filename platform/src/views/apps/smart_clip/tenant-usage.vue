...
<script lang="ts" setup name="platform-smart-clip-tenant-usage">
import { getSmartClipTenantStat } from '@/apps/smart_clip/api'
...
const getData = async () => {
    loading.value = true
    try {
        stat.value = await getSmartClipTenantStat({ tenant_id: tenantId.value || 0 })
    } finally {
        loading.value = false
    }
}
