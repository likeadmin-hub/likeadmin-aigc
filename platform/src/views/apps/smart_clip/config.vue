<template>
    <div>
        <el-card class="!border-none" shadow="never">
            <div class="text-lg font-medium">AI视频剪辑平台配置</div>
            <div class="text-sm text-tx-secondary mt-1">平台默认供应商配置，租户未单独配置时使用平台配置。</div>
        </el-card>
...
<script lang="ts" setup name="platform-smart-clip-config">
import { getSmartClipPlatformConfig, getSmartClipTenantStat, setSmartClipPlatformConfig } from '@/apps/smart_clip/api'
...
const getData = async () => {
    loading.value = true
    try {
        Object.assign(formData, await getSmartClipPlatformConfig())
        stat.value = await getSmartClipTenantStat()
    } finally {
        loading.value = false
    }
}
const handleSubmit = async () => {
    await setSmartClipPlatformConfig(formData)
    getData()
}
getData()
</script>
