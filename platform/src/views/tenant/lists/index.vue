    ...
    <div>
        ...
        <el-card class="!border-none mt-4" shadow="never">
            ...
            <el-table class="mt-4" size="large" v-loading="pager.loading" :data="pager.lists">
                ...
                <el-table-column label="租户点数" prop="point_balance" min-width="110" />
                            ...
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        ...
                                        <el-dropdown-item
                                            v-if="hasPermission(['tenant.tenant/sso'])"
                                            @click="handleEnterTenant(row)"
                                        >
                                            进入后台
                                        </el-dropdown-item>
                                        <el-dropdown-item
                                            v-if="hasPermission(['tenant.tenant/rechargePoints'])"
        ...
        <el-dialog v-model="rechargeVisible" title="租户点数充值" width="420px">
            <el-form label-width="90px" :model="rechargeForm">
                <el-form-item label="租户">
                    {{ rechargeForm.tenant_name }}
                </el-form-item>
                <el-form-item label="当前点数">
                    {{ rechargeForm.current_points }}
                </el-form-item>
                <el-form-item label="充值点数">
                    <el-input-number v-model="rechargeForm.points" :min="0.01" :precision="2" class="!w-full" />
                </el-form-item>
...
import { getUserList, tenantPointRecharge, tenantSso, userDelete } from '@/api/consumer'
...
const openRecharge = (row: any) => {
    rechargeForm.tenant_id = row.id
    rechargeForm.tenant_name = row.name
    rechargeForm.current_points = row.point_balance || '0.00'
    rechargeForm.points = 1
    rechargeForm.remark = ''
    rechargeVisible.value = true
}

const handleRecharge = async () => {
    await tenantPointRecharge({
        tenant_id: rechargeForm.tenant_id,
...
const handleEnterTenant = async (row: any) => {
