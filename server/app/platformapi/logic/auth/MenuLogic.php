...
class MenuLogic extends BaseLogic
{
    ...
    private static function filterSystemServiceMenus(array $menus): array
    {
        ...
        return array_values(array_filter($menus, function ($menu) use ($allowedKeys) {
            $key = (string)($menu['source_menu_key'] ?? '');
            return !str_starts_with($key, 'core_update_') || in_array($key, $allowedKeys, true);
        }));
    }
    ...
    private static function sortTopLevelTailMenus(array $menus): array
    {
        ...
        foreach ($menus as $index => $menu) {
            $indexedMenus[] = [
                'index' => $index,
                'is_tail' => self::isSystemSettingMenu($menu),
                'menu' => $menu,
            ];
        }
     ...
     * @return bool
     */
    private static function isSystemSettingMenu(array $menu): bool
    {
        return (string)($menu['paths'] ?? '') === 'setting' || (string)($menu['name'] ?? '') === '系统设置';
    }
