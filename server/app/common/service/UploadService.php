    ...
    public static function image($cid, int $sourceId = 0, int $source = FileEnum::SOURCE_ADMIN, string $saveDir = 'uploads/images')
    {
        try {
            ...
            $file = (AdminTerminalEnum::isPlatform() ? new File() : new TenantFile())->create([
                ...
                'storage_scope' => $config['scope'] ?? 'platform',
                'storage_engine' => $config['default'] ?? 'local',
                'storage_domain' => StorageConfigService::getEffectiveDomain(StorageConfigService::currentTenantId()),
                'source'      => $source,
                'source_id'   => $sourceId,
                'create_time' => time(),
            ]);
            ...
            return [
                ...
                'type' => $file['type'],
                'name' => $file['name'],
                'uri'  => FileService::getFileUrlByStorage($file['uri'], $file['storage_scope'] ?? '', $file['storage_engine'] ?? '', $file['storage_domain'] ?? ''),
                'url'  => $file['uri']
            ];
