...
class StorageController extends BaseAdminController
{
    public function lists()
    {
        ...
        return $this->success('获取成功', [
            ...
            'lists' => [
                ['name' => '本地存储', 'path' => '存储在本地服务器', 'engine' => 'local', 'status' => $enabled && $default === 'local' ? 1 : 0],
    ...
    public function change()
    {
        ...
        $default = $this->getConfig('default', 'local');
        StorageSettingService::set('tenant', $this->tenantId, 'enable', 1);
        StorageSettingService::set('tenant', $this->tenantId, 'default', $default === $engine ? 'local' : $engine);
        StorageConfigService::clearCache($this->tenantId);
