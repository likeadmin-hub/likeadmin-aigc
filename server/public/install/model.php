...
class installModel
{
    ...
    private bool $hadTestingTableBeforeInstall = false;
...
//        }
        ...
        $databaseExists = (bool)$this->dbExists();
        $this->hadTestingTableBeforeInstall = $databaseExists ? (bool)$this->tableExits() : false;
        if ( !$databaseExists) {
        ...
        } elseif ($this->hadTestingTableBeforeInstall and $this->clearDB == false) {
            $return->result = 'fail';
            $return->error = '数据表已存在，您之前可能已安装本系统。如确认是上次安装失败残留，请勾选清空当前数据库中同前缀的数据表后重新安装。';
            return $return;
        } elseif ($databaseExists and $this->clearDB == true) {
            if (!$this->dropTablesByPrefix()) {
                $return->result = 'fail';
                $return->error = '数据表已经存在，清空已存在数据表错误,请手动清除';
                return $return;
            }
            $this->hadTestingTableBeforeInstall = false;
        }

        if (!$this->selectDb()) {
            $return->result = 'fail';
            $return->error = '选择数据库错误';
            return $return;
        }

        /* Create tables. */
        if ( !$this->createTable($version, $connectionInfo)) {
            $this->cleanupFailedInstall($connectionInfo['name']);
            $return->result = 'fail';
            $return->error = '创建表格失败' . ($this->lastSqlError ? '：' . $this->lastSqlError : '');
            return $return;
        }
    ...
    public function createTable($version, $post)
    {
        ...
        foreach ($tables as $table) {
            $table = $this->normalizeSqlStatement($table);
...
//            }
    ...
    private function normalizeSqlStatement(string $sql): string
    {
        $lines = preg_split('/\r\n|\r|\n/', trim($sql));
        $statements = [];
    ...
    {
        $sql = "drop database {$this->quoteIdentifier($db)};";
        return $this->dbh->query($sql);
    }

    private function selectDb(): bool
    {
        return (bool)$this->dbh->query('USE ' . $this->quoteIdentifier($this->name));
    }

    /**
     * Notes: 安装失败时回滚本次安装写入的数据
     * @param string $db
     * @return void
     */
    public function cleanupFailedInstall(string $db): void
    {
        try {
            if ($this->createdDatabase) {
                $this->dropDb($db);
                return;
            }

            if ($this->clearDB || !$this->hadTestingTableBeforeInstall) {
                $this->dropTablesByPrefix();
            }
        } catch (Exception $e) {
    ...
    private function dropTablesByPrefix(): bool
    {
        $like = $this->escapeLike($this->prefix) . '%';
        $sql = "SHOW TABLES FROM {$this->quoteIdentifier($this->name)} LIKE " . $this->dbh->quote($like);
        $tables = $this->dbh->query($sql)->fetchAll(PDO::FETCH_COLUMN);
        if (empty($tables)) {
            return true;
        }
        ...
        $tableNames = [];
        foreach ($tables as $table) {
            $tableNames[] = $this->quoteIdentifier($this->name) . '.' . $this->quoteIdentifier($table);
        }

        $this->dbh->exec('SET FOREIGN_KEY_CHECKS=0');
        try {
            $this->dbh->exec('DROP TABLE IF EXISTS ' . implode(',', $tableNames));
        } finally {
