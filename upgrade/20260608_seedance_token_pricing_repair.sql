...
ON DUPLICATE KEY UPDATE
    ...
    `la_aigc_video_channel_spec`.`cost_source_url` = IF(`la_aigc_video_channel_spec`.`cost_source_url` IS NULL OR `la_aigc_video_channel_spec`.`cost_source_url` = '', VALUES(`cost_source_url`), `la_aigc_video_channel_spec`.`cost_source_url`),
