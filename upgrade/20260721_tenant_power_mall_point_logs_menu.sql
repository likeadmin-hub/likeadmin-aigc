...
VALUES
('system_default','power.mall/consumeLogs','GET','power_mall:consume_logs','tenant_admin',1,1,1,UNIX_TIMESTAMP(),UNIX_TIMESTAMP()),
('system_default','power.mall/consumeLogDetail','GET','power_mall:consume_log_detail','tenant_admin',1,1,1,UNIX_TIMESTAMP(),UNIX_TIMESTAMP())
...
SET child.`pid` = parent.`id`,
    child.`name` = '点数流水',
    child.`icon` = 'el-icon-Notebook',
    child.`sort` = 80,
    child.`perms` = 'power.mall/consumeLogs',
    child.`paths` = 'consume-logs',
    child.`component` = 'power_mall/consume_logs',
    child.`is_show` = 1,
    child.`is_disable` = 0,
...
WHERE consume_logs.`source_menu_key` = 'core_tenant_power_mall_consume_logs'
  AND NOT EXISTS (
    SELECT 1 FROM `la_tenant_system_menu` exists_menu
...
SET child.`pid` = consume_logs.`id`,
    child.`perms` = 'power.mall/consumeLogDetail',
    child.`is_show` = 0,
    child.`is_disable` = 0,
    child.`update_time` = UNIX_TIMESTAMP()
